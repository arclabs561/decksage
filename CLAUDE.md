...
Training sources (OK for embedding and reranker training):
...
- Self-supervised pairs from attribute overlap (`data/training/selfsupervised_pairs.json`)
  Generated by `scripts/training/generate_selfsupervised_pairs.py`
...
Exception: **reranker training** may use annotations IF eval bypasses the reranker (evaluates raw embedding quality separately). Document the leakage clearly and use query-level held-out splits.
