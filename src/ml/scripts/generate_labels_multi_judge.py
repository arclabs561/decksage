...
# ///
"""
Generate labels using multiple LLM judges for IAA tracking.

Uses 3-5 LLM judges per query and computes inter-annotator agreement.
...
# Import from existing labeling script
import sys
...
try:
    ...
    HAS_ENHANCED = True
    HAS_LABELING_SCRIPTS = True
    
    def generate_labels_for_query_with_retry(agent, query, use_case=None, game=None, max_retries=3):
        """Wrapper for enhanced generation."""
        # Always load card context (with game if available)
    ...
    try:
        ...
        HAS_LABELING_SCRIPTS = False
        logger.warning(f"Could not import labeling scripts: {e}")

# Try to use LLM cache if available
try:
    import sys
    from pathlib import Path as P
...
def generate_labels_multi_judge(
    ...
    Returns:
        Dict with labels (majority vote) and IAA metrics
    """
    if not HAS_PYDANTIC_AI:
        logger.error("pydantic-ai required")
        return {}
    
    if not HAS_LABELING_SCRIPTS:
        logger.error("Labeling scripts not available")
        return {}
    
    # Generate labels from multiple judges
    all_judgments = []
    for judge_id in range(num_judges):
        ...
        if labels and any(labels.values()):
    ...
    # First, validate each judgment for internal contradictions (same card in multiple levels)
    # This is a quality check - a single judge shouldn't contradict themselves
    validated_judgments = []
    ...
    # Majority vote: card appears in >= 50% of judgments
    # Note: Judges may disagree (that's expected and measured by IAA)
    # But each judge should be internally consistent (no contradictions)
    card_votes: dict[str, dict[str, int]] = {}  # card -> {level: count}
    ...
    # Assign cards to levels based on majority vote
    # If judges disagree, use the level with most votes (majority wins)
    final_labels = {
        "highly_relevant": [],
        "relevant": [],
        "somewhat_relevant": [],
    ...
    threshold = len(validated_judgments) / 2  # Majority
    ...
    # This should be integrated, not post-processing
    validated_labels = final_labels.copy()
    
    # Only use heuristic if game not explicitly provided
    # In production, game should always be passed as parameter
    if not game:
        try:
            # Check if we can infer game from query name patterns
            query_lower = query.lower()
            if any(x in query_lower for x in ["energy", "pokemon", "pikachu", "charizard"]):
                game = "pokemon"
            elif any(x in query_lower for x in ["blue-eyes", "dark magician", "exodia", "pot of"]):
                game = "yugioh"
            else:
                game = "magic"  # Default
        except Exception:
            game = "magic"  # Default fallback
    
    # Filter cross-game cards if game is known and card database available
    # NOTE: This happens after majority vote, which is suboptimal but necessary
    # because we need to collect all judgments first. In future, could filter
    # during judgment collection, but that would require game to be known upfront.
    if game:
        try:
            ...
            total_filtered = 0
            for level in ["highly_relevant", "relevant", "somewhat_relevant", "marginally_relevant"]:
                cards = validated_labels[level]
                valid_cards, invalid_cards = card_db.filter_cards_by_game(cards, game)
                if invalid_cards:
                    total_filtered += len(invalid_cards)
                    logger.warning(f"Filtered {len(invalid_cards)} cross-game cards from {level} for {query}: {invalid_cards[:3]}")
                validated_labels[level] = valid_cards
            
            if total_filtered > 0:
                logger.info(f"Total cross-game cards filtered for {query}: {total_filtered}")
        except ImportError:
            # Card database not available, skip filtering
            logger.warning("Card database not available, skipping cross-game filtering")
        except Exception as e:
            logger.warning(f"Could not filter cross-game cards: {e}")
    
    # Compute agreement metrics
    # Try to use Krippendorff's Alpha if available, fall back to simple agreement
    try:
        from ml.evaluation.krippendorff_alpha import compute_iaa_for_labels
        
        iaa_metrics = compute_iaa_for_labels(validated_judgments)
    except ImportError:
        # Fall back to simple agreement calculation
        agreement_scores = []
        for card, votes in card_votes.items():
            total_votes = sum(votes.values())
            max_votes = max(votes.values()) if votes else 0
            agreement = max_votes / len(validated_judgments) if validated_judgments else 0.0
        ...
        iaa_metrics = {
            ...
            "krippendorff_alpha": None,  # Not available
        }
...
def main() -> int:
    """Generate labels with multi-judge IAA tracking."""
    ...
    for query_name, query_data in queries_needing_labels:
        use_case = query_data.get("use_case")
        logger.info(f"Generating labels for {query_name} ({args.num_judges} judges)...")
        
        result = generate_labels_multi_judge(query_name, args.num_judges, use_case)
        ...
        processed += 1
        iaa = result.get("iaa", {})
        logger.info(f"  ✅ Generated labels (IAA: {iaa.get('agreement_rate', 0.0):.2f}, {iaa.get('num_judges', 0)} judges)")
