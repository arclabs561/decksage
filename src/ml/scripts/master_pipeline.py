...
def main() -> int:
    ...
    parser.add_argument(
        "--skip-downstream", action="store_true", help="Skip downstream test data creation"
    )
    parser.add_argument("--skip-training", action="store_true", help="Skip training")
    parser.add_argument("--skip-eval", action="store_true", help="Skip evaluation")
    parser.add_argument(
        "--embedding", type=str, help="Embedding file to evaluate (if skipping training)"
    )
    ...
    if not args.skip_vocab:
        for game in ["magic", "pokemon", "yugioh"]:
            ...
            pairs_csv = (
                PATHS.pairs_large if game == "magic" else (PATHS.processed / f"pairs_{game}.csv")
            )
            ...
            if test_set.exists() and pairs_csv.exists():
                success = run_command(
                    [
                        ...
                        "run",
                        "--script",
                        "src/ml/scripts/fix_vocabulary_mismatch.py",
                        "--test-set",
                        str(test_set),
                        "--pairs-csv",
                        str(pairs_csv),
                        "--name-mapping",
                        "experiments/name_mapping.json",
                        "--threshold",
                        "0.80",
                    ],
    ...
    if not args.skip_expand:
        success = run_command(
            [
                ...
                "run",
                "--script",
                "src/ml/scripts/expand_test_set_multi_game.py",
                "--games",
                "magic",
                "pokemon",
                "yugioh",
            ...
            if test_set.exists() and pairs_csv.exists():
                success = run_command(
                    [
                        ...
                        "run",
                        "--script",
                        "src/ml/scripts/create_downstream_test_data.py",
                        "--game",
                        game,
                        "--pairs-csv",
                        str(pairs_csv),
                        ...
                        str(test_set),
                        "--output-dir",
                        "experiments/downstream_tests",
                    ],
                    f"Creating downstream test data for {game}",
                )
                if not success:
                    print(
                        f"Warning: Downstream test data creation failed for {game}, continuing..."
                    )
        ...
        if Path(embedding_path).exists():
            success = run_command(
                [
                    ...
                    "run",
                    "--script",
                    "src/ml/scripts/comprehensive_evaluation_pipeline.py",
                    "--embedding",
                    embedding_path,
                    "--output",
                    str(PATHS.experiments / "evaluation_comprehensive.json"),
                    "--test-sets",
                    f"magic:{PATHS.test_magic}",
