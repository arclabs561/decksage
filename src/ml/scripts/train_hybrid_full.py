...
from ..utils.logging_config import setup_script_logging, log_exception, log_progress
...
def build_incremental_graph(
    ...
    logger.info(f" Train: {len(train_decks):,} decks ({train_decks[0]['_parsed_timestamp'] if train_decks else 'N/A'} to {train_decks[-1]['_parsed_timestamp'] if train_decks else 'N/A'})")
    logger.info(f" Val: {len(val_decks):,} decks ({val_decks[0]['_parsed_timestamp'] if val_decks else 'N/A'} to {val_decks[-1]['_parsed_timestamp'] if val_decks else 'N/A'})")
    logger.info(f" Test: {len(test_decks):,} decks ({test_decks[0]['_parsed_timestamp'] if test_decks else 'N/A'} to {test_decks[-1]['_parsed_timestamp'] if test_decks else 'N/A'}) [EXCLUDED from graph]")
    ...
    if use_temporal_split:
        try:
            ...
            is_valid, warnings = validate_temporal_split(decks_to_use, train_frac=train_frac, val_frac=val_frac)
            if is_valid:
        ...
        logger.warning("Warning: WARNING: Building graph from ALL decks - potential data leakage if test period included")
...
def train_gnn_embeddings(
 ...
 game: str | None = None,
    ...
    logger.info(f"Initializing GNN embedder (type={model_type}, dim={hidden_dim}, layers={num_layers})...")
...
def setup_instruction_embeddings(model_name: str = "intfloat/e5-base-v2") -> InstructionTunedCardEmbedder:
...
def evaluate_components(
 ...
 test_set_path: Path,
    ...
    for query in sample_queries:
        labels = test_set[query].get("labels", []) if isinstance(test_set[query], dict) else test_set[query]
        if labels:
...
try:
 ...
 try:
 from ml.utils.training_registry import register_training_run
 except ImportError: from ..utils.training_registry import register_training_run
 ...
 logger.info(f"✓ Hybrid training run registered")
 except Exception as e:
