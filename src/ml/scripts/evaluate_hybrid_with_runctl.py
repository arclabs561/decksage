...
def evaluate_hybrid(
    ...
    if graph_path:
        ...
        if graph_path.exists():
            ...
            logger.info(
                f" Full graph: {len(full_graph.nodes)} nodes, {len(full_graph.edges)} edges"
            )

            if use_temporal_split:
                logger.info("Applying temporal split to prevent data leakage...")
                splitter = TemporalSplitter(
                    SplitConfig(train_frac=0.7, val_frac=0.15, test_frac=0.15)
                )
                ...
                logger.info(
                    f" Test graph: {len(test_graph.edges)} edges [EXCLUDED from evaluation]"
                )
                ...
                logger.warning(
                    "Warning: WARNING: Using full graph for Jaccard - potential data leakage"
                )
    ...
    embeddings_data = load_hybrid_embeddings(
        ...
        cooccurrence_embeddings_path=Path(cooccurrence_embeddings_path)
        if cooccurrence_embeddings_path
        else None,
    ...
    try:
        ...
        if output_path:
            version = (
                output_path.stem.split("_v")[-1]
                if "_v" in output_path.stem
                else datetime.now().strftime("%Y-W%V")
            )
