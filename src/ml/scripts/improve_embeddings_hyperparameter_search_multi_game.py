...
def main() -> int:
    ...
    parser.add_argument(
        "--input", type=str, required=True, help="Multi-game pairs CSV (S3 or local)"
    )
    parser.add_argument(
        "--output", type=str, required=True, help="Output results JSON (S3 or local)"
    )
    parser.add_argument(
        "--test-sets", type=str, nargs="+", help="Test set paths: game1:path1 game2:path2"
    )
    parser.add_argument(
        "--max-configs", type=int, default=20, help="Maximum configurations to test"
    )
