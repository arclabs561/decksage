...
# ]
# ///
"""Optimize fusion weights with more combinations (chunked)."""
import json
import sys
...
# Load best embedding
embedding = KeyedVectors.load("data/embeddings/trained_contrastive_substitution.wv")
...
# Test more weight combinations
weight_combinations = [
    (0.1, 0.9),  # Very low embed, very high Jaccard
    (0.2, 0.8),  # Low embed, high Jaccard
    (0.3, 0.7),  # Medium-low embed, medium-high Jaccard
    (0.4, 0.6),  # Balanced
    (0.5, 0.5),  # Equal
    (0.6, 0.4),  # High embed, low Jaccard
    (0.7, 0.3),  # Very high embed, low Jaccard
    (0.8, 0.2),  # Very high embed, very low Jaccard
]
...
# Save
output_path = Path("experiments/fusion_optimization_extended.json")
