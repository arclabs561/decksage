...
Measures P@10 for each similarity signal independently:
- Embedding signal alone
- Jaccard signal alone
- Functional signal alone

Then compares to fusion to understand signal contributions.
"""
...
def evaluate_embedding_signal(
    wv: KeyedVectors,
    test_set: dict[str, dict[str, Any]],
    top_k: int = 10,
) -> dict[str, Any]:
    """Evaluate embedding signal alone (match evaluate_all_embeddings methodology)."""
    correct = 0
    total = 0
    reciprocal_ranks = []
    relevance_weights = {
    ...
    for query, labels in test_set.items():
        ...
        # Precision@K (weighted by relevance) - match evaluate_all_embeddings
        score = 0.0
    ...
    return {
        "p@10": float(precision),
        "mrr": float(np.mean(reciprocal_ranks)),
        "num_queries": total,
    }


def evaluate_jaccard_signal(
    adj: dict[str, set[str]],
    test_set: dict[str, dict[str, Any]],
    top_k: int = 10,
) -> dict[str, Any]:
    """Evaluate Jaccard signal alone (match evaluate_all_embeddings methodology)."""
    ...
    for query, labels in test_set.items():
        ...
        # Compute Jaccard similarity to all neighbors
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Measure individual signal performance")
    parser.add_argument(
        "--test-set",
        type=str,
    ...
    print("=" * 70)
    print("Measure Individual Signal Performance")
    print("=" * 70)
    print()
    ...
    # Evaluate embedding signal
    embed_path = Path(args.embeddings)
    if embed_path.exists():
        print("Evaluating embedding signal...")
        try:
            wv = KeyedVectors.load(str(embed_path))
            metrics = evaluate_embedding_signal(wv, test_set)
            results["embedding"] = metrics
            print(f"  P@10: {metrics['p@10']:.4f}, MRR: {metrics['mrr']:.4f}, Queries: {metrics['num_queries']}")
        except Exception as e:
            print(f"  ❌ Error: {e}")
            results["embedding"] = {"error": str(e)}
    else:
        print(f"⚠️  Embeddings not found: {embed_path}")
        results["embedding"] = {"error": "File not found"}
    print()
    
    # Evaluate Jaccard signal
    pairs_csv = Path(args.pairs_csv)
    if pairs_csv.exists():
        print("Evaluating Jaccard signal...")
        try:
            adj = load_graph_for_jaccard(pairs_csv)
            metrics = evaluate_jaccard_signal(adj, test_set)
        ...
        print(f"⚠️  Pairs CSV not found: {pairs_csv}")
        results["jaccard"] = {"error": "File not found"}
    print()
    
    # TODO: Functional signal (needs FunctionalTagger)
    print("⚠️  Functional signal not yet implemented (needs FunctionalTagger)")
    results["functional"] = {"error": "Not implemented"}
    print()
    ...
    print(f"{'Signal':<20} {'P@10':<10} {'MRR':<10} {'Queries':<10}")
    print("-" * 50)
    for signal, metrics in results.items():
        if "error" not in metrics:
            print(
                f"{signal:<20} {metrics.get('p@10', 0.0):<10.4f} "
                f"{metrics.get('mrr', 0.0):<10.4f} {metrics.get('num_queries', 0):<10}"
            )
        else:
            print(f"{signal:<20} {'ERROR':<10}")
