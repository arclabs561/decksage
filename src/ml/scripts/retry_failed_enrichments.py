...
# ///
"""
Retry failed card enrichments with improved name matching.
...
def normalize_card_name(name: str) -> str:
    ...
    # Remove special formatting but keep essential characters
...
def get_name_variants(card_name: str) -> list[str]:
    ...
    if no_parens != card_name and no_parens:
    ...
    if ascii_only != card_name and ascii_only.strip():
    ...
    if cleaned != card_name and cleaned:
        variants.append(cleaned)
...
def get_card_from_scryfall_with_retries(
    card_name: str,
    delay: float = ADAPTIVE_DELAY,
    max_retries: int = 3,
) -> tuple[dict[str, Any] | None, float]:
    """Get card data with multiple retry strategies."""
    ...
    for attempt, variant in enumerate(variants[:max_retries]):
        ...
        try:
            ...
            params = {"exact": variant, "format": "json"}
            ...
            if response.status_code == 404:
                ...
                params = {"fuzzy": variant, "format": "json"}
                response = requests.get(url, params=params, timeout=10)
                if response.status_code == 200:
                    return response.json(), delay
            ...
            if response.status_code == 429:
                ...
                if retry_after:
                    try:
                        new_delay = float(retry_after) + 0.1
                        logger.warning(f"Rate limited, waiting {new_delay:.3f}s")
            ...
            delay = min(MAX_DELAY, delay * 1.2)
            continue
        except Exception as e:
            logger.debug(f"Error fetching {variant}: {e}")
            continue
...
def retry_failed_enrichments(
    ...
    """Retry enrichment for failed cards."""
    logger.info(f"Loading attributes from {input_csv}...")
    
    df = pd.read_csv(input_csv, dtype=str)
    
    # Find failed cards
    failed_mask = df["type"].isna() | (df["type"] == "") | (df["type"] == "nan")
    failed_indices = df[failed_mask].index.tolist()
    
    logger.info(f"Found {len(failed_indices)} failed cards to retry")
    ...
    for idx in failed_indices:
        ...
        logger.info(f"Retrying: {card_name}")
        
        card_data, current_delay = get_card_from_scryfall_with_retries(
            card_name, current_delay, max_retries
        )
        ...
        if card_data:
            ...
            enriched += 1
            logger.info(f"  ✅ Enriched: {card_name}")
        else:
            still_failed += 1
            logger.warning(f"  ❌ Still failed: {card_name}")
        
        # Save periodically
        if enriched % 10 == 0:
            df.to_csv(output_csv, index=False)
            logger.info(f"  💾 Progress saved ({enriched} retried successfully)")
    ...
    logger.info(f"   Success rate: {100*enriched/(enriched+still_failed):.1f}%" if (enriched+still_failed) > 0 else "N/A")


def main() -> int:
    """Retry failed enrichments."""
    parser = argparse.ArgumentParser(description="Retry failed card enrichments")
    ...
    parser.add_argument("--max-retries", type=int, default=3, help="Max retry attempts per card")
    ...
    retry_failed_enrichments(input_path, output_path, args.max_retries)
