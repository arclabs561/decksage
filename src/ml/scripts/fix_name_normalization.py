...
# ///
"""
Fix name normalization issues between test set and embeddings/graph.

Issue: Test set uses canonical names, but embeddings/graph may use different names.
Need to normalize names consistently.
"""
    ...
    for query, labels in test_set.items():
        ...
        if missing_in_embed or missing_in_graph:
            mismatches["relevant_cards_not_found"][query] = {
                "missing_in_embeddings": missing_in_embed[:5],  # Limit for readability
                "missing_in_graph": missing_in_graph[:5],
    ...
    if mismatches["relevant_cards_not_found"]:
        ...
        for query, info in list(mismatches["relevant_cards_not_found"].items())[:3]:
            ...
            if info["missing_in_embeddings"]:
                print(f"    Missing in embeddings: {info['missing_in_embeddings']}")
