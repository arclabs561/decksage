...
def infer_similarity_type(relevance: int, notes: str, query_context: dict, cand_context: dict) -> str | None:
    ...
    if rel_int >= 3:
        ...
        if "substitute" in notes_lower or "replace" in notes_lower or "same function" in notes_lower:
            return "functional"
...
def infer_role_match(similarity_type: str | None, query_context: dict, cand_context: dict) -> bool | None:
    """Infer role_match from similarity_type and card contexts."""
    if similarity_type == "functional":
        return True
    elif similarity_type == "synergy":
        return False
    elif similarity_type == "unrelated":
        return False
    else:
        return None
...
def retrofit_annotation_batch(input_path: Path, output_path: Path | None = None) -> Path:
    ...
    for task in tasks:
        ...
        for cand in candidates:
            ...
            if all(cand.get(field) is not None for field in 
                   ["similarity_type", "is_substitute", "role_match"]):
                continue
...
def main() -> int:
    ...
    parser.add_argument("--output", type=str, help="Output YAML file (default: input.retrofitted.yaml)")
