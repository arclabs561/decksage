...
def evaluate_embedding_on_game(embedding_name: str, embedding_path: str, game: str, test_set_path: str):
    """Evaluate an embedding on a game."""
    print(f"  Evaluating {embedding_name} on {game}...")
    ...
    try:
        ...
        # Extract results (handle different return formats)
        ...
        mrr_ci = results.get("mrr_ci", results.get("mrr@10_ci", [0.0, 0.0]))
        
        return {
            "p@10": p_at_10,
            "p@10_ci": p_at_10_ci if isinstance(p_at_10_ci, list) else [p_at_10_ci, p_at_10_ci],
            "mrr": mrr,
            "mrr_ci": mrr_ci if isinstance(mrr_ci, list) else [mrr_ci, mrr_ci],
...
if __name__ == "__main__":
    ...
    for embedding_name, embedding_path in EMBEDDINGS.items():
        ...
        if not embedding_path_obj.exists():
            print(f"⏭️  Skipping {embedding_name}: Not found")
            continue
        
        all_results[embedding_name] = {}
        
        for game, test_set_path in TEST_SETS.items():
            test_set_path_obj = Path(test_set_path)
            if not test_set_path_obj.exists():
                print(f"⏭️  Skipping {game}: Test set not found")
                continue
            
            result = evaluate_embedding_on_game(embedding_name, embedding_path, game, test_set_path)
            if result:
                all_results[embedding_name][game] = result
    
    # Save results
    output_path = Path("experiments/multi_game_evaluation.json")
    with open(output_path, "w") as f:
        json.dump(all_results, f, indent=2)
    
    print()
    print("=" * 70)
    print("MULTI-GAME EVALUATION RESULTS")
    print("=" * 70)
    print()
    
    for embedding_name, game_results in all_results.items():
        print(f"{embedding_name}:")
        for game, result in game_results.items():
            p_at_10 = result["p@10"]
            ci = result["p@10_ci"]
            print(f"  {game:10s}: P@10={p_at_10:.3f} [{ci[0]:.3f}, {ci[1]:.3f}]")
        print()
