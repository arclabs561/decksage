...
# ]
# ///
"""Optimize fusion weights using heterogeneous as baseline."""
import json
import sys
...
# Load heterogeneous (best performer)
heterogeneous = KeyedVectors.load("data/embeddings/trained_heterogeneous_substitution.wv")
print(f"✅ Loaded heterogeneous: {len(heterogeneous)} cards")
...
# Test fusion combinations with heterogeneous
weight_combinations = [
    (0.0, 1.0),  # Pure Jaccard
    (0.1, 0.9),  # Mostly Jaccard
    (0.2, 0.8),
    (0.3, 0.7),
    (0.4, 0.6),
    (0.5, 0.5),  # Equal
...
print("Testing fusion with heterogeneous baseline...")
results = {}
...
for embed_weight, jaccard_weight in weight_combinations:
    ...
    for i in range(0, len(test_pairs), chunk_size):
        ...
        for original, target in chunk:
            if original not in heterogeneous or original not in adj:
                continue
...
print()
print("=" * 70)
print("BEST FUSION WEIGHTS (with heterogeneous)")
print("=" * 70)
print(f"Combination: {best[0]}")
print(f"P@10: {best[1]['p@10']:.3f}")
print(f"Found: {best[1]['found']}/{best[1]['total']}")
print(f"Baseline (heterogeneous alone): P@10=0.420")

# Save
output_path = Path("experiments/fusion_optimization_heterogeneous.json")
