...
def integrate_attribute_relationships(
    ...
    graph_cards = {
        name: node for name, node in graph.nodes.items() if not game or node.game == game
    }

    # Only process cards that are in both graph and attributes
    cards_to_process = set(graph_cards.keys()) & set(card_attributes.keys())
    logger.info(f"Processing {len(cards_to_process):,} cards with both graph and attribute data")

    if not cards_to_process:
        logger.warning("No cards found in both graph and attributes")
        return stats
    ...
    for card_name in cards_to_process:
        attrs = card_attributes[card_name]
    ...
    logger.info(
        f"Created/updated {stats['total_edges_created'] + stats['total_edges_updated']} attribute edges"
    )
