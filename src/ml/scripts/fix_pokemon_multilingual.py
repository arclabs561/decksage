        ...
        if response.status_code == 200:
            ...
            if isinstance(data, list):
                            ...
                            if card_id:
                                try:
                                    en_response = requests.get(
                                        f"{TCGDEX_API_BASE}/en/cards", timeout=5
                                    )
                                    if en_response.status_code == 200:
                                        ...
                                        if isinstance(en_data, list):
                                            for en_card in en_data[:500]:  # Limit search
                                                if (
                                                    en_card.get("id") == card_id
                                                    or en_card.get("localId") == card_id
                                                ):
                ...
                for card in data[:500]:  # Limit for performance
                    ...
                    if card_name_lower in localized_lower or localized_lower in card_name_lower:
                        return {
                            "name": card.get("name"),
                            "localized_name": localized,
                            "language": language,
        ...
        if card_data:
            ...
            if english_name:
                ...
                if game and game.lower() == "pokemon":
                    ...
                    if fixed <= 10:
                        logger.info(
                            f"  ✓ {card_name} ({detected_lang}) -> {english_name} -> PKM (API)"
                        )
                else:
                    ...
                    if detected_lang in [
                        ...
                        if fixed <= 10:
                            logger.info(
                                f"  ✓ {card_name} ({detected_lang}) -> {english_name} -> PKM (assumed)"
                            )
