...
# ///
"""
Optimize fusion weights using test set evaluation.

Uses scipy.optimize to find optimal weights for combining:
...
After embeddings improve, run this to find best fusion weights.
"""
...
def load_embeddings(embed_path: Path):
    ...
    try:
        ...
        return KeyedVectors.load(str(embed_path))
...
def evaluate_fusion_weights(
    ...
    """
    Evaluate fusion weights and return negative P@10 (for minimization).
    
    Weights: [embed_weight, jaccard_weight, ...]
    """
    embed_weight = weights[0]
    jaccard_weight = weights[1]
    
    # Normalize weights
    total = embed_weight + jaccard_weight
    if total == 0:
    ...
    for query, labels in test_set.items():
        if query not in wv and query not in adj:
            continue
        
        # Get embedding scores
        embed_scores = {}
        if query in wv and embed_weight > 0:
            try:
                similar = wv.most_similar(query, topn=top_k * 2)
                for card, sim in similar:
                    embed_scores[card] = float(sim) * embed_weight
            except KeyError:
                pass
        
        # Get Jaccard scores
        jaccard_scores = {}
        if query in adj and jaccard_weight > 0:
            ...
            for candidate in adj.keys():
                ...
                jaccard_scores[candidate] = sim * jaccard_weight
        ...
        for candidate in all_candidates:
            combined_scores[candidate] = (
                embed_scores.get(candidate, 0.0) +
                jaccard_scores.get(candidate, 0.0)
            )
        
        # Get top-k
        top_candidates = sorted(combined_scores.items(), key=lambda x: x[1], reverse=True)[:top_k]
        candidates = [card for card, _ in top_candidates]
...
def optimize_fusion(
    ...
    if not HAS_OPTIMIZATION:
        logger.error("scipy required for optimization")
        return {}
    
    # Load data
    logger.info("Loading embeddings...")
    wv = load_embeddings(embed_path)
    if not wv:
        return {}
    
    logger.info("Loading Jaccard graph...")
    adj = load_jaccard_graph(pairs_csv)
    
    logger.info("Loading test set...")
    with open(test_set_path) as f:
        data = json.load(f)
    test_set = data.get("queries", data) if isinstance(data, dict) else data
    
    logger.info(f"Optimizing fusion weights on {len(test_set)} queries...")
    ...
    final_score = -evaluate_fusion_weights(optimal_weights, wv, adj, test_set, top_k)
    
    logger.info(f"✅ Optimization complete!")
    logger.info(f"   Optimal weights: embed={optimal_weights[0]:.3f}, jaccard={optimal_weights[1]:.3f}")
    logger.info(f"   Final P@10: {final_score:.4f}")
    
    return {
        "embed_weight": float(optimal_weights[0]),
        "jaccard_weight": float(optimal_weights[1]),
        "p@10": float(final_score),
...
def main() -> int:
    ...
    parser.add_argument("--output", type=str, default="experiments/optimal_fusion_weights.json", help="Output JSON")
