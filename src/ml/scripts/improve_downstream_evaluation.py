...
# ///
"""
Improved downstream task evaluation using fusion.

Uses embedding + Jaccard fusion for better performance.
"""
...
# Try to load functional tagger (for Magic)
try:
...
def load_jaccard_graph(pairs_csv: Path) -> dict[str, set[str]]:
    ...
    df = pd.read_csv(pairs_csv, nrows=100000)  # Sample for speed
    adj = {}
...
def evaluate_with_fusion(
    ...
    """Evaluate downstream task using fusion (embedding + Jaccard)."""
    if not HAS_DOWNSTREAM:
        return {"error": "Downstream modules not available"}
    
    # Load Jaccard graph
    print("  Loading Jaccard graph...")
    adj = load_jaccard_graph(pairs_csv)
    print(f"  Loaded {len(adj)} cards in Jaccard graph")
    
    # Create fusion with functional tags and Oracle text if available
    tagger = None
    if _HAS_TAGGER and game == "magic":
        try:
            from ml.enrichment.card_functional_tagger import FunctionalTagger
    ...
    # Try to load Oracle text embedder
    text_embedder = None
    try:
    ...
    if card_attrs_path.exists() and text_embedder:
        try:
            import pandas as pd
            ...
            for _, row in df.iterrows():
                ...
                card_data[name] = {
                    ...
                    "type_line": str(row.get("type", "")),  # Use type_line for CardTextEmbedder
    ...
    # Use functional tags and text embeddings if available
    if tagger and text_embedder:
        weights = FusionWeights(embed=0.25, jaccard=0.35, functional=0.20, text_embed=0.20)
    elif tagger:
        weights = FusionWeights(embed=0.3, jaccard=0.4, functional=0.3)
    elif text_embedder:
        weights = FusionWeights(embed=0.3, jaccard=0.4, text_embed=0.3)
    else:
        weights = FusionWeights(embed=0.4, jaccard=0.6)  # Favor Jaccard slightly
    ...
    def candidate_fn(card: str, k: int) -> list[tuple[str, float]]:
        """Candidate function using fusion."""
        try:
            results = fusion.similar(card, k=k)
            return results  # Already returns list of (card, score) tuples
        except Exception:
    ...
    if task == "deck_completion":
        test_decks = test_data.get("decks", [])
        results = {
    ...
    elif task == "card_substitution":
        test_pairs = test_data.get("pairs", [])
        results = {
        ...
        for original, target in test_pairs:
            try:
                suggestions = suggest_replacements(
                    ...
                    card=original,
                    candidate_fn=candidate_fn,
                    top_k=50,  # Check even more candidates for evaluation
                )
        ...
        if results["found_substitute"] > 0:
            results["avg_rank"] = np.mean(ranks) if ranks else 0.0
    ...
    elif task == "contextual_discovery":
        test_queries = test_data.get("queries", [])
        
        # Create discovery with fusion
...
def main() -> int:
    """Evaluate downstream tasks with fusion."""
    parser = argparse.ArgumentParser(description="Evaluate downstream tasks with fusion")
    ...
    if args.test_decks:
        decks_path = Path(args.test_decks)
        if decks_path.exists():
            print("\n📊 Evaluating deck completion with fusion...")
            test_decks = []
            with open(decks_path) as f:
                for line in f:
                    test_decks.append(json.loads(line))
            
            completion_results = evaluate_with_fusion(
                embedding=embedding,
                pairs_csv=pairs_path,
                test_data={"decks": test_decks},
                game=args.game,
                task="deck_completion",
            )
            ...
            print(f"  Avg quality: {completion_results.get('avg_quality', 0):.4f}")
    
    # Evaluate card substitution
    if args.test_substitutions:
        subs_path = Path(args.test_substitutions)
        if subs_path.exists():
            print("\n📊 Evaluating card substitution with fusion...")
            with open(subs_path) as f:
                test_pairs = json.load(f)
            
            substitution_results = evaluate_with_fusion(
                embedding=embedding,
                pairs_csv=pairs_path,
                test_data={"pairs": test_pairs},
                game=args.game,
                task="card_substitution",
            )
            results["tasks"]["card_substitution"] = substitution_results
            print(f"  P@10: {substitution_results.get('p@10', 0):.4f}")
            print(f"  Found: {substitution_results.get('found_substitute', 0)}/{substitution_results.get('total_pairs', 0)}")
    
    # Evaluate contextual discovery
    if args.test_contextual:
        contextual_path = Path(args.test_contextual)
        if contextual_path.exists():
            print("\n📊 Evaluating contextual discovery with fusion...")
            with open(contextual_path) as f:
                test_queries = json.load(f)
            
            discovery_results = evaluate_with_fusion(
                embedding=embedding,
                pairs_csv=pairs_path,
                test_data={"queries": test_queries},
                game=args.game,
                task="contextual_discovery",
            )
