...
def check_test_set_size(game: str = "magic") -> dict[str, Any]:
    ...
    for q in queries.values():
        if isinstance(q, dict):
            ...
            has_labels = any(
                q.get(level, [])
                for level in ["highly_relevant", "relevant", "somewhat_relevant", "labels"]
            )
            if has_labels:
...
def generate_quality_dashboard() -> dict[str, Any]:
    ...
    health = compute_system_health(
        ...
        completion_validation_path=completion_validation
        if completion_validation.exists()
        else None,
...
def validate_text_embeddings_integration() -> dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "message": "Text embeddings integrated"
            if embedder_available
            else "Text embeddings not available",
        }
...
def validate_ab_testing_framework() -> dict[str, Any]:
    ...
    try:
        from ml.evaluation.ab_testing import ABTestConfig, ABTestFramework
        ...
        return {
            ...
            "message": "A/B testing framework available"
            if has_methods
            else "A/B testing framework partially available",
        }
    except ImportError as e:
        return {
            "status": "warn",
    ...
    if args.check_prerequisites and not args.skip_prerequisites:
        ...
        try:
            ...
            if prereq_results["overall"] == "fail":
                logger.error(
                    "Prerequisites check failed - some required dependencies/files missing"
                )
