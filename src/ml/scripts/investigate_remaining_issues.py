...
def analyze_mismatched_edges(graph_db: Path, limit: int = 100) -> dict[str, Any]:
    ...
    for ex in examples:
        logger.info(
            f"  {ex['card1']} <-> {ex['card2']}: edge={ex['edge_game']}, n1={ex['node1_game']}, n2={ex['node2_game']}"
        )
...
async def agentic_analysis(
    edge_analysis: dict[str, Any], node_analysis: dict[str, Any], graph_db: Path
) -> None:
        ...
        if nodes_differ > 0:
            ...
            if examples:
                ...
                for ex in examples:
                    ...
                    if "//" in ex["card2"]:
                        logger.info(
                            f"    → '{ex['card2']}' is a split card, likely mislabeled as YGO"
                        )

            logger.info(
                "Root Cause: Cards from different games connected (likely name collision or split card mislabeling)"
            )
        ...
        if not_found > 0:
            ...
            if examples:
                ...
                for ex in examples:
                    ...
                    if "//" in ex["name"]:
                        logger.info("    → Split card - may need special normalization")
                    elif len(ex["name"]) < 5:
                        logger.info("    → Very short name - may be abbreviation or typo")

            logger.info(
                "Root Cause: Card names not matching database (normalization, abbreviations, or new cards)"
            )
            logger.info("Recommended Fix:")
            if has_slash > 0:
            ...
            logger.info(
                "  5. For split cards: Ensure both halves are checked (e.g., 'Fire // Ice' → check 'Fire' and 'Ice')"
            )
...
def main() -> int:
    ...
    if total_time > 60:
        logger.warning(
            f"Analysis took {total_time:.1f}s - consider optimizing card database loading"
        )
