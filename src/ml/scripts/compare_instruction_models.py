    ...
    for model_name in models:
        ...
        try:
            ...
            if model_results["tested_queries"] > 0:
                model_results["accuracy"] = model_results["correct_predictions"] / model_results["tested_queries"]
                avg_sim = sum(s["similarity"] for s in model_results["similarities"]) / len(model_results["similarities"])
            ...
            logger.info(f"✓ {model_name} tested: {model_results['tested_queries']} queries, accuracy: {model_results['accuracy']:.2%}")
