...
def load_trained_assets(
    ...
    for weights_path in [
        ...
        if weights_path.exists():
            try:
                ...
                logger.info(
                    f"  Loaded fusion weights: embed={fusion_weights.embed:.2f}, jaccard={fusion_weights.jaccard:.2f}, functional={fusion_weights.functional:.2f}"
                )
                break
            except Exception as e:
    ...
    if fusion_weights is None:
        ...
        if fast_mode or disable_functional:
            ...
            logger.info("  Using fast mode weights (functional tags disabled)")
        else:
            fusion_weights = FusionWeights(embed=0.4, jaccard=0.4, functional=0.2).normalized()
            logger.info("  Using default fusion weights (optimized for substitution)")
...
def evaluate_deck_completion_task(
    ...
    for i, deck in enumerate(test_decks):
        try:
            ...
            results["failed"] += 1
            results["errors"].append(f"Deck {i}: {e!s}")
...
def evaluate_deck_refinement_task(
    ...
    refiner = DeckRefiner(
        fusion=assets[
            "fusion"
        ],  # Will use task_type="completion" for adds, "substitution" for replaces
    ...
    for i, deck in enumerate(test_decks):
        try:
            ...
            log_exception(
                logger, f"Failed to refine deck {i}", e, level="warning", include_context=True
            )
...
def critique_performance(
        ...
        if comp.get("completed", 0) > 0:
            ...
            if avg_improvement > 1.0:
                critique["strengths"].append(
                    f"Deck completion improves quality by {avg_improvement:.2f} points on average"
                )
            elif avg_improvement < 0:
                critique["weaknesses"].append(
                    f"Deck completion degrades quality (avg improvement: {avg_improvement:.2f})"
                )
                critique["recommendations"].append(
                    "Review completion algorithm - may be adding wrong cards"
                )
    ...
    if "substitution" in results:
        ...
        if p_at_10 > 0.5:
            ...
            critique["recommendations"].append(
                "Improve substitution by using functional tags more heavily"
            )
    ...
    if "refinement" in results:
        ...
        if ref.get("refined", 0) > 0:
            ...
            if avg_add > 5:
                critique["strengths"].append(
                    f"Refinement generates {avg_add:.1f} add suggestions on average"
                )
            elif avg_add < 2:
                critique["weaknesses"].append(
                    f"Refinement generates few suggestions ({avg_add:.1f})"
                )
    ...
    if len(critique["strengths"]) > len(critique["weaknesses"]):
        ...
        critique["overall_assessment"] = (
            "Multiple issues identified - needs significant improvement"
        )
    else:
        critique["overall_assessment"] = (
            "Mixed performance - some tasks work well, others need work"
        )
...
def main() -> int:
    ...
    parser.add_argument(
        "--fast",
        action="store_true",
        help="Fast mode: disable expensive operations (functional tags, text embeddings)",
    )
    parser.add_argument(
        "--disable-functional", action="store_true", help="Disable functional tag similarity"
    )
    parser.add_argument(
        "--disable-text-embed", action="store_true", help="Disable text embedding similarity"
    )
    parser.add_argument(
        "--use-runctl",
        action="store_true",
        help="Auto-detect if running on AWS and use runctl optimizations",
    )
    parser.add_argument(
        "--min-pairs-for-runctl",
        type=int,
        default=1000,
        help="Minimum pairs to trigger runctl recommendation",
    )
    parser.add_argument(
        "--validate-deck-quality",
        action="store_true",
        help="Run deck quality validation after evaluation",
    )
    parser.add_argument(
        "--generate-dashboard",
        action="store_true",
    ...
    is_aws = (
        args.use_runctl or Path("/mnt/data").exists() or os.getenv("AWS_EXECUTION_ENV") is not None
    )
    if is_aws:
                ...
                if isinstance(data, dict) and "queries" in data:
                    ...
                    for query_card, labels in queries.items():
                        if isinstance(labels, dict):
                            ...
                            for target in (highly_relevant + relevant)[
                                :3
                            ]:  # Up to 3 pairs per query
    ...
    if test_pairs_count > args.min_pairs_for_runctl:
        ...
        logger.warning("   Consider using runctl on AWS for faster execution:")
        logger.warning(f"   ./scripts/evaluation/validate_e2e_runctl.sh --game {args.game}")
        if not args.use_runctl and not is_aws:
            logger.warning("   Or add --use-runctl flag to enable AWS optimizations")
    ...
    if args.validate_deck_quality:
        try:
            ...
            if similarity_fn:
                ...
                logger.info(
                    f"Deck quality validation: {quality_results['success_rate'] * 100:.1f}% success rate"
                )
            else:
                logger.warning(
                    "Cannot run deck quality validation: similarity function not available"
                )
        except Exception as e:
    ...
    if args.generate_dashboard:
        try:
            ...
            health = compute_system_health(
                completion_validation_path=local_output_path
                if args.validate_deck_quality
                else None,
    ...
    try:
        ...
        registry.record_evaluation(
            ...
            metadata={
                ...
                "test_substitutions": str(args.test_substitutions)
                if args.test_substitutions
                else None,
