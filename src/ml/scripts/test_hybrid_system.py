...
def test_gnn_incremental() -> bool:
    ...
    try:
        ...
        graph.add_deck({"cards": [{"name": "Lightning Strike", "count": 4}, {"name": "Lightning Bolt", "count": 4}]})
...
def test_fusion_integration() -> bool:
    ...
    try:
        ...
        embeddings_data = load_hybrid_embeddings(
            gnn_model_path=PATHS.embeddings / "test_gnn.json" if (PATHS.embeddings / "test_gnn.json").exists() else None,
        )
