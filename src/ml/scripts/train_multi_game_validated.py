...
# ///
"""
Train multi-game embeddings with similarity-based validation.

Uses test sets from all games for validation.
"""
...
try:
    from ml.scripts.train_multi_game_embeddings import (
        load_multi_game_pairs,
        create_game_aware_walks,
    )
    HAS_MULTI_GAME = True
except ImportError:
    HAS_MULTI_GAME = False
...
def train_with_validation(
    ...
    """Train multi-game embeddings with validation."""
    if not HAS_DEPS or not HAS_MULTI_GAME:
        logger.error("Missing dependencies")
        return None
    
    # Load multi-game pairs
    pairs_csvs = {"MTG": pairs_csv}  # Can add YGO, PKM later
    adj, weights, game_map = load_multi_game_pairs(pairs_csvs)
    
    logger.info(f"Loaded graph: {len(adj)} nodes")
    
    # Create game-aware walks
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Train multi-game embeddings with validation")
    parser.add_argument("--input", type=str, required=True, help="Multi-game pairs CSV")
    ...
    parser.add_argument("--test-sets", type=str, nargs="*", help="Test sets (game:path format)")
