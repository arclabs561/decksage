...
class LeakageAnalyzer:
    ...
    def analyze_graph_construction(self, graph_path: Path, decks_path: Path) -> list[LeakageIssue]:
        ...
        if decks_path.exists():
            ...
            if timestamps:
                ...
                graph_edge_times = [edge.first_seen for edge in graph.edges.values()]
                if graph_edge_times:
    ...
    def analyze_test_set_construction(
        self, test_set_path: Path, graph_path: Path
    ) -> list[LeakageIssue]:
        """
        ...
        if test_set_path.exists():
            ...
            test_queries = (
                test_data.get("queries", test_data) if isinstance(test_data, dict) else test_data
            )
    ...
    def validate_temporal_split(
        ...
        if train_max_ts >= test_min_ts:
            warnings.append(
                f"Temporal split violation: train period ({train_max_ts}) overlaps with test period ({test_min_ts})"
            )
            return False, warnings
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--decks", type=Path, default=PATHS.decks_all_final, help="Decks JSONL path"
    )
    parser.add_argument(
        "--test-set", type=Path, default=PATHS.test_magic, help="Test set JSON path"
    )
    parser.add_argument(
        "--pairs", type=Path, default=PATHS.pairs_large, help="Pairs CSV path (optional)"
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=PATHS.experiments / "leakage_analysis_report.json",
