    ...
    if not path.exists():
        ...
        if str(path).startswith("s3://"):
            ...
            result = subprocess.run(
                ["s5cmd", "cp", str(path), "/tmp/temp_eval.json"],
                capture_output=True,
                text=True
            )
...
def compare_versions(
    ...
    print("\nKey Metrics:")
    print(f" P@10: {previous_metrics.get('p_at_10', 0):.4f} → {current_metrics.get('p_at_10', 0):.4f} ({deltas.get('p_at_10_delta_pct', 0):+.1f}%)")
    print(f" P@5: {previous_metrics.get('p_at_5', 0):.4f} → {current_metrics.get('p_at_5', 0):.4f} ({deltas.get('p_at_5_delta_pct', 0):+.1f}%)")
    print(f" Completion: {previous_metrics.get('completion_rate', 0):.4f} → {current_metrics.get('completion_rate', 0):.4f} ({deltas.get('completion_rate_delta_pct', 0):+.1f}%)")
