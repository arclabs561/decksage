...
class SimilarityMethod:
    ...
    def cosine(wv, card1: str, card2: str) -> float:
        """Cosine similarity (default for word2vec)"""
        try:
...
class ModelComparator:
    ...
    def evaluate_model(
        ...
        Metrics:
        - Precision@K (graded: highly_relevant = 1.0, relevant = 0.75, etc)
        ...
        for query, ground_truth in self.test_set.items():
            ...
            # Get all test cards for this query
            all_test_cards = (
                ground_truth.get("highly_relevant", [])
                + ground_truth.get("relevant", [])
                + ground_truth.get("somewhat_relevant", [])
                + ground_truth.get("marginally_relevant", [])
                + ground_truth.get("irrelevant", [])
            )
            ...
            for k in k_values:
                # Precision@K (graded)
                p_at_k = self._precision_at_k(scored_cards[:k], ground_truth)
                results[f"P@{k}"].append(p_at_k)

                # NDCG@K
                ndcg = self._ndcg_at_k(scored_cards[:k], ground_truth, k)
                results[f"NDCG@{k}"].append(ndcg)

            # MRR
            mrr = self._mrr(scored_cards, ground_truth)
    ...
    def _precision_at_k(self, predictions: list, ground_truth: dict) -> float:
        """Graded precision - weight by relevance level"""
        relevance_weights = {
            "highly_relevant": 1.0,
            "relevant": 0.75,
    ...
    def _mrr(self, predictions: list, ground_truth: dict) -> float:
        """Mean reciprocal rank - rank of first highly relevant"""
    ...
    def compare_models(
        self, models: dict[str, str], similarity_methods: dict[str, Callable] | None = None
    ) -> pd.DataFrame:
        """
        Compare multiple models side-by-side.

        Args:
            models: {name: path_to_wv_file}
            similarity_methods: {name: similarity_function}
        ...
        for model_name, wv_path in models.items():
            ...
            for sim_name, sim_fn in similarity_methods.items():
                ...
                metrics = self.evaluate_model(wv, sim_fn)

                results.append({"model": model_name, "similarity": sim_name, **metrics})
...
def main():
    ...
    results_df = comparator.compare_models(models, similarity_fns)
    ...
    print(f"\n🏆 Best model: {best['model']} ({best['similarity']})")
...
<head>
    ...
    <style>
        ...
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .bar {{ background: var(--accent); height: 18px; border-radius: 2px; display: inline-block; }}
    </style>
</head>
<body>
    <div class="container">
        ...
        <div class="winner">
            <h2>Winner: {best["model"]} ({best["similarity"]})</h2>
            <p><span class="metric">P@10: {best["P@10"]:.4f}</span> | <span class="metric">NDCG@10: {best["NDCG@10"]:.4f}</span> | <span class="metric">MRR: {best["MRR"]:.4f}</span></p>
        </div>
