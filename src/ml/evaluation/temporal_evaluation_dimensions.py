...
def evaluate_temporal_appropriateness(
    ...
    return TemporalEvaluation(
        temporal_context_appropriate=(legal_at_time and price_reasonable and meta_relevant),
...
def _identify_temporal_issues(
    ...
    if not legal:
        issues.append("Card was not legal at recommendation time")

    if not price_reasonable:
        issues.append("Price was not reasonable at recommendation time")

    if not meta_relevant:
        issues.append("Card was not meta-relevant at recommendation time")
    ...
    if card in current_ban_list and legal:
        issues.append("Card was legal when recommended but got banned later")
    ...
    if price_then and price_now and price_now > price_then * 2:
        issues.append(
            f"Card price spiked after recommendation ({price_then:.2f} → {price_now:.2f})"
        )
...
def _identify_context_changes(
    ...
    if card in ban_list_now and card not in ban_list_then:
        changes.append("Card was banned after recommendation")
