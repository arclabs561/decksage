...
class TemporalSplitter:
    ...
    def split_decks(
        ...
        sorted_decks = sorted(
            ...
            key=lambda d: datetime.fromisoformat(
                d.get(
                    "timestamp", d.get("created_at", d.get("date", datetime.now().isoformat()))
                ).replace("Z", "+00:00")
            ),
        )
        ...
        logger.info(
            f"Temporal split: {len(train_decks)} train, {len(val_decks)} val, {len(test_decks)} test"
        )
    ...
    def split_graph_edges(
        ...
        sorted_edges = sorted(graph.edges.items(), key=lambda x: x[1].first_seen)
...
class AblationStudy:
    ...
    def generate_ablation_configs(self) -> list[dict[str, bool]]:
        ...
        # All components
        configs.append(dict.fromkeys(components, True))

        # Baseline (none)
        configs.append(dict.fromkeys(components, False))
    ...
    def create_fusion_weights(
        ...
        if total == 0:
            ...
            weights = {c: weight_per if c in enabled else 0.0 for c in component_config}
        else:
            weights = {
    ...
    for i, component_config in enumerate(configs):
        ...
        try:
            ...
            if component_config.get("instruction"):
                try:
                    from ml.similarity.instruction_tuned_embeddings import (
            ...
            if test_set and test_set.exists():
                ...
                logger.info(
                    f"  Evaluation: P@10={metrics['p_at_10']:.4f}, MRR={metrics['mrr']:.4f}"
                )
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=PATHS.experiments / "ablation_results.json",
        help="Output path",
    )
