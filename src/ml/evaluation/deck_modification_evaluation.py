...
class DeckModificationEvaluator:
    ...
    def generate_test_cases(self) -> tuple[list[DeckModificationTestCase], list[ContextualDiscoveryTestCase]]:
        """Generate test cases based on critiques."""
        deck_cases = []
        contextual_cases = []
        
        # Test case 1: Empty deck (should suggest core staples)
        deck_cases.append(DeckModificationTestCase(
            name="empty_burn_deck",
            game="magic",
            deck={"partitions": [{"name": "Main", "cards": []}]},
            archetype="Burn",
            format="Modern",
            expected_additions=["Lightning Bolt", "Lava Spike", "Rift Bolt", "Monastery Swiftspear"],
            expected_removals=[],
        ...
        contextual_cases.append(ContextualDiscoveryTestCase(
...
def main():
    ...
    parser.add_argument("--generate-annotations", action="store_true", help="Also generate annotation templates")
    ...
    # Save results
    output_path = Path(args.output) if args.output else PATHS.experiments / "deck_modification_critique.json"
