...
PRICING_DATA = {
    ...
    "openai/gpt-5.2": {
        "input": 5.00,
        "output": 15.00,
    },  # Estimated (GPT-5.2 pricing not yet public)
...
class LLMCostTracker:
    ...
    def print_summary(self):
        ...
        print(
            f"Cache Hits: {summary.cache_hits} ({summary.cache_hits / summary.total_calls * 100:.1f}%)"
        ...
        if summary.by_model:
            ...
            for model, stats in sorted(
                summary.by_model.items(), key=lambda x: x[1]["cost_usd"], reverse=True
            ):
                print(f"  {model}:")
        ...
        if summary.by_operation:
            ...
            for op, stats in sorted(
                summary.by_operation.items(), key=lambda x: x[1]["cost_usd"], reverse=True
            ):
                print(f"  {op}:")
                print(f"    Calls: {stats['calls']} (hits: {stats['cache_hits']})")
                print(f"    Tokens: {stats['input_tokens']:,} in, {stats['output_tokens']:,} out")
