    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                logger.debug(
                    f"HTTP error for {card_name} (attempt {attempt + 1}/{max_retries}): {e}"
                )
            ...
            if attempt < max_retries - 1:
                logger.debug(
                    f"Error fetching {card_name} (attempt {attempt + 1}/{max_retries}): {e}"
                )
                time.sleep(2**attempt)
            else:
...
def enrich_card_attributes_with_images(
    ...
    logger.info(
        f"  (This will take ~{len(cards_to_process) * 0.1 / 60:.1f} minutes due to rate limiting)"
    )
    ...
    for i, (card_name, attrs) in enumerate(cards_to_process):
        ...
        if (i + 1) % batch_size == 0:
            logger.info(
                f"  Progress: {i + 1}/{len(cards_to_process)} ({fetched} fetched, {failed} failed)"
            )
...
__all__ = ["enrich_card_attributes_with_images", "get_scryfall_image_url"]
