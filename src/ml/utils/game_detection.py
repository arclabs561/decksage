...
def detect_game_from_card_name(card_name: str) -> str | None:
    ...
    # One Piece patterns
    if any(x in name_lower for x in ["luffy", "zoro", "nami", "sanji", "don!!", "leader", "one piece"]):
        return "onepiece"
    
    # Riftbound patterns
    if any(x in name_lower for x in ["champion", "domain", "body", "calm", "chaos", "fury", "mind", "order", "riftbound"]):
        return "riftbound"
