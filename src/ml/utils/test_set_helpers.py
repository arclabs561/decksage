...
def ensure_test_set_size(
    ...
    if not auto_expand:
        logger.warning(
            f"Test set {test_set_path} has only {current_size} queries (minimum: {min_size})"
        )
        return test_set_path
            ...
            if current_size < min_queries:
                ...
                if expanded_path != test_set_path:
                    ...
                    try:
                        ...
                        queries = (
                            expanded_data.get("queries", expanded_data)
                            if isinstance(expanded_data, dict)
                            else expanded_data
                        )
                        test_set_data = (
                            expanded_data
                            if isinstance(expanded_data, dict) and "queries" in expanded_data
