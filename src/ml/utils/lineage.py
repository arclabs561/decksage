...
def is_order_0_location(path: str | Path) -> bool:
    ...
    path_str = str(path)
    return any(path_str.startswith(loc.rstrip("/")) or loc.rstrip("/") in path_str 
               for loc in ORDER_0_LOCATIONS)


def validate_write_path(path: str | Path, order: int) -> tuple[bool, Optional[str]]:
    """
...
def get_order_for_path(path: str | Path) -> Optional[int]:
