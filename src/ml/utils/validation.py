...
def check_instance_status(instance_id: str, runctl_bin: Path | str | None = None) -> dict[str, Any]:
    ...
    runctl_bin = Path(runctl_bin) if runctl_bin else PATHS.project_root.parent / "runctl" / "target" / "release" / "runctl"
...
def validate_evaluation_prerequisites(
 ...
 if graph_path:
     ...
     if graph_path_obj.exists():
         ...
         results["warnings"].append(f"Graph not found: {graph_path} (Jaccard similarity disabled)")
         results["checks"]["graph"] = "⚠"
