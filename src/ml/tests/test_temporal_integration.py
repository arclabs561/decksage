...
class TestDeckMetadataToTemporalTracking:
    ...
    def test_metadata_extraction_and_temporal_tracking(self, temp_graph, nested_deck):
        ...
        temp_graph.set_deck_metadata(deck_id, {
            "format": "Modern",
...
class TestEdgeCases:
    ...
    def test_deck_with_round_results(self, temp_graph, deck_with_round_results):
        ...
        temp_graph.add_deck(deck_with_round_results, timestamp=datetime(2024, 1, 15), deck_id=deck_id)
    ...
    def test_nested_structure(self, temp_graph, nested_deck):
        ...
        format_value = (
            nested_deck.get("format")
            or (nested_deck.get("type", {}).get("inner", {}) if isinstance(nested_deck.get("type"), dict) else {}).get("format")
        )
