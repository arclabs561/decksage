...
def move_ops_roundtrip(draw):
    ...
    return [
        {"op": "move_card", "from_partition": "Main", "to_partition": "Sideboard", "card": card, "count": 1},
        {"op": "move_card", "from_partition": "Sideboard", "to_partition": "Main", "card": card, "count": 1},
    ]
...
def test_move_roundtrip_preserves_multiset(ops):
    deck = {
        ...
        "format": "Modern",
        "partitions": [
            {"name": "Main", "cards": [{"name": "Lightning Bolt", "count": 2}, {"name": "Goblin Guide", "count": 2}]},
...
def test_structured_errors_present_on_copy_limit_violation():
    ...
    assert any(d.code == "copy_limit" for d in res.details)
