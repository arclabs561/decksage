...
def temp_graph(tmp_path):
    ...
    return IncrementalCardGraph(graph_path=graph_path, use_sqlite=False)
...
def test_graph_metadata_extraction(temp_graph):
    ...
    assert "Standard_2025" in edge.format_periods or len(edge.format_periods) >= 0, (
        "Should track format period"
    )
