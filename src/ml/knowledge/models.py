...
class ArchetypeDefinition(BaseModel):
    ...
    core_cards: list[str] = Field(
        default_factory=list, description="Essential cards (70%+ inclusion)"
    )
    ...
    interaction_level: str | None = Field(
        None, description="Interaction level (heavy/medium/light)"
    )
...
class GameKnowledge(BaseModel):
    ...
    archetypes: list[ArchetypeDefinition] = Field(
        default_factory=list, description="Archetype definitions"
    )
    ...
    temporal_context: dict[str, Any] = Field(
        default_factory=dict, description="Temporal context (meta, bans, etc.)"
    )
