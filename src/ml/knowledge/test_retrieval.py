...
def test_retrieval_logic():
    ...
    knowledge_file = (
        Path(__file__).parent.parent.parent.parent / "data" / "game_knowledge" / "magic.json"
    )
    ...
    try:
        ...
        matching_examples = [
            ex
            for ex in examples
            if query_lower in ex.get("card1", "").lower()
            or query_lower in ex.get("card2", "").lower()
        ]
        ...
        if matching_examples:
            ...
            print(
                f"    Example: {ex.get('card1')} vs {ex.get('card2')} (score: {ex.get('score', 'N/A')})"
            )
