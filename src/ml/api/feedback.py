...
class FeedbackRequest(BaseModel):
    ...
    context: dict[str, Any] | None = Field(
        None, description="Optional context (deck, format, archetype, etc.)"
    )
...
class BatchFeedbackRequest(BaseModel):
    ...
    feedbacks: list[FeedbackRequest] = Field(
        ..., description="List of feedback entries", max_length=100
    )
...
def _check_rate_limit(user_id: str | None, session_id: str | None) -> tuple[bool, str | None]:
    ...
    _rate_limit_storage[identifier] = [
        t for t in _rate_limit_storage[identifier] if now - t < RATE_LIMIT_WINDOW
    ]
    ...
    if len(_rate_limit_storage[identifier]) >= RATE_LIMIT_MAX_REQUESTS:
        return (
            False,
            f"Rate limit exceeded: {RATE_LIMIT_MAX_REQUESTS} requests per {RATE_LIMIT_WINDOW}s",
        )
...
def _validate_card_name(card_name: str, game: str | None = None) -> tuple[bool, str | None]:
    ...
    try:
        ...
        logger.warning(
            f"Card '{card_name}' not found in embeddings/database (validation non-fatal)"
        )
...
def store_feedback(feedback: dict[str, Any]) -> str:
    ...
    try:
        ...
        logger.info(
            f"Stored feedback: {feedback['feedback_id']} ({feedback.get('task_type', 'unknown')})"
        )
    except OSError as e:
...
def submit_feedback(req: FeedbackRequest) -> FeedbackResponse:
    ...
    try:
        ...
        if not query_card or not suggested_card:
            raise HTTPException(
                status_code=400, detail="query_card and suggested_card are required"
            )
        ...
        invalid_chars = [
            c for c in query_card + suggested_card if ord(c) < 32 and c not in "\t\n\r"
        ]
        if invalid_chars:
            raise HTTPException(
                status_code=400, detail="Card names contain invalid control characters"
            )
...
def submit_batch_feedback(req: BatchFeedbackRequest) -> BatchFeedbackResponse:
    ...
    if len(req.feedbacks) == 0:
        raise HTTPException(
            status_code=400, detail="Batch must contain at least one feedback entry"
        )
    ...
    if not allowed:
        ...
        now = time()
        current_count = len(
            [t for t in _rate_limit_storage.get(identifier, []) if now - t < RATE_LIMIT_WINDOW]
        )
