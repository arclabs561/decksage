...
class Edge:
    ...
    metadata: dict[str, Any] | None = None  # Format, placement, event_date, archetype, partition, similarity scores
...
class IncrementalCardGraph:
    ...
    def __init__(
        ...
        if self.graph_path and self.graph_path.exists():
            ...
            if self.graph_path.suffix == ".db" or (self.use_sqlite and self.graph_path.suffix == ".db"):
    ...
    def save_sqlite(self, path: Path | str | None = None) -> None:
        ...
        try:
            ...
            if "database is locked" in error_msg.lower():
                raise RuntimeError(
                    f"Database is locked during save. Another process may be using it. "
                    "Wait for it to finish or close other connections."
                ) from e
            elif "database disk image is malformed" in error_msg.lower():
                raise RuntimeError(
                    f"Database appears corrupted during save. "
                    "Try restoring from backup or rebuilding the graph."
                ) from e
            else:
    ...
    def load_sqlite(self, path: Path | str) -> None:
        ...
        try:
            ...
            try:
                cursor = self._db_conn.execute("SELECT card1, card2, game, weight, first_seen, last_seen, deck_sources, metadata, monthly_counts, format_periods FROM edges")
            except sqlite3.OperationalError:
                # Fallback for old schema without temporal columns
                cursor = self._db_conn.execute("SELECT card1, card2, game, weight, first_seen, last_seen, deck_sources, metadata FROM edges")
            ...
            for i in range(0, len(rows), chunk_size):
                ...
                for row in chunk:
                    if len(row) >= 10:
                        ...
                        card1, card2, game, weight, first_seen, last_seen, deck_sources, metadata, monthly_counts, format_periods = row
                    else:
                        # Old schema without temporal data
                        card1, card2, game, weight, first_seen, last_seen, deck_sources, metadata = row
                        monthly_counts = None
                        format_periods = None
                    ...
                    self.edges[edge_key] = Edge(
                        ...
                        card2=card2,
                        game=game,
                        weight=int(weight) if isinstance(weight, (int, float)) else weight,  # Ensure weight is numeric
                        first_seen=datetime.fromisoformat(first_seen) if isinstance(first_seen, str) else first_seen,
                        last_seen=datetime.fromisoformat(last_seen) if isinstance(last_seen, str) else last_seen,
            ...
            if "database is locked" in error_msg.lower():
                ...
                raise RuntimeError(f"Failed to load from SQLite database {path}: {error_msg}") from e
        except Exception as e:
    ...
    def load_json(self, path: Path | str) -> None:
        ...
        # Load nodes
        self.nodes = {
            name: CardNode.from_dict(node_data)
