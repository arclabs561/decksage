    ...
    *,
    ...
    if tag_set_fn is not None and coverage_weight > 0:
        ...
        metrics = {
            ...
            "score_reasons": {
                card: score_reasons.get(card, "Similarity match") for card, _ in boosted[:5]
            },
        }
    ...
    metrics = {
        ...
        "role_counts": role_counts,
        "score_reasons": {
            card: score_reasons.get(card, "Similarity match") for card, _ in legal[:5]
        },
    }
        ...
        try:
            ...
            quality_metrics = {
                "initial": {
                    ...
                    "mana_curve_score": initial_quality.mana_curve_score
                    if initial_quality
                    else None,
                    "tag_balance_score": initial_quality.tag_balance_score
                    if initial_quality
                    else None,
                ...
                "improvement": final_quality.overall_score
                - (initial_quality.overall_score if initial_quality else 0.0),
            }
                ...
                if not other_archetypes:
                    ...
                    removals.append(
                        (
                            card,
                            0.5,
                            f"not_archetype_staple (in {len(other_archetypes)} other archetypes)",
                        )
                    )
            else:
...
def suggest_replacements(
    ...
    *,
    ...
    for replacement, similarity_score in candidates:
        ...
        role_overlap = (
            len(current_role & replacement_role) / len(current_role | replacement_role)
            if (current_role | replacement_role)
            else 0.0
        )
