...
class ContextualCardDiscovery:
                ...
                for neighbor in query_neighbors:
                    ...
                    if archetype and self.archetype_cooccurrence:
                        ...
                        if neighbor_cooccur_rate > 0.3:  # Lower threshold
                            ...
                            reasoning_parts.append(f"high archetype co-occurrence ({neighbor_cooccur_rate:.0%})")
                    ...
                    if format and self.format_cooccurrence:
                        ...
                        if format_cooccur_rate > 0.3:  # Lower threshold
                            ...
                            reasoning_parts.append(f"high format co-occurrence ({format_cooccur_rate:.0%})")
    ...
    def find_alternatives(
        ...
        try:
            ...
            for alt_card, embed_score in similar:
                    ...
                    role_overlap = len(current_role & alt_role) / len(current_role | alt_role) if (current_role | alt_role) else 0.0
    ...
    def find_upgrades(
        ...
        for alt in alternatives:
            ...
            score = alt.score * (1.0 + min(price_delta / 10.0, 0.3))  # Up to 30% boost for expensive upgrades
    ...
    def find_downgrades(
        ...
        for alt in alternatives:
            ...
            score = alt.score * (1.0 + min(savings_pct * 0.5, 0.4))  # Up to 40% boost for big savings
            
            reasoning = f"budget alternative (${current_price:.2f} → ${alt_price:.2f}, save ${abs(price_delta):.2f})"
            
            downgrades.append(CardDowngrade(
                card=alt.card,
                score=score,
                price_delta=price_delta,
                reasoning=reasoning,
