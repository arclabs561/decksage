...
class CardSuggestion:
    ...
    reasoning: str
    impact: dict[str, Any]
    target: str | None = None  # For replace/move actions
    ...
    def suggest_additions(
        ...
        if constraints.archetype:
            ...
            archetype_staples_list = [
                (card, rate)
                for card, rate in archetype_staples_list
            ...
            try:
                ...
                for seed_card in seed_cards:
                    try:
                        ...
                        candidates = self.fusion.similar(
                            seed_card, k=constraints.max_suggestions * 3, task_type="completion"
                        )
                        for card, score in candidates:
                            ...
                            if canonical_card not in current_cards:
                                ...
                                candidate_scores[canonical_card] = max(
                                    candidate_scores.get(canonical_card, 0.0), float(score)
                                )
                    except Exception:
                ...
                log_exception(
                    logger,
                    "Failed to get fusion suggestions",
                    e,
                    level="warning",
        ...
        sorted_scored = sorted(scored.items(), key=lambda x: x[1][0], reverse=True)[
            : constraints.max_suggestions
        ]
        logger.debug(
            f"Final scored suggestions: {len(sorted_scored)} (from {len(scored)} total scored)"
        )
        ...
        for card, (score, reason) in sorted_scored:
            suggestions.append(
                CardSuggestion(
                    ...
                    reasoning=reason,
                    impact={
                        "role_gaps_filled": role_gaps,
                        "archetype_match": constraints.archetype or "none",
                    },
                )
            )
        ...
        if not suggestions:
            logger.warning(
                f"No suggestions generated - scored={len(scored)}, fusion={len(fusion_suggestions)}, archetype={len(archetype_staples_list)}"
            )
                ...
                if score > 0.0:
                    suggestions.append(
                        CardSuggestion(
                            ...
                            if reason_parts
                            else "better suited for sideboard",
