...
class GraphQualityAgent:
    ...
    def _collect_statistics(self) -> None:
        ...
        logger.info(
            f"Statistics collected: {self.statistics['total_nodes']:,} nodes, {self.statistics['total_edges']:,} edges"
        )
    ...
    def _check_data_quality(self) -> None:
        ...
        conn = sqlite3.connect(str(self.graph_db))
        zero_weight_count = conn.execute("SELECT COUNT(*) FROM edges WHERE weight = 0").fetchone()[
            0
        ]
        conn.close()
    ...
    def _check_completeness(self) -> None:
        ...
        missing_temporal_pct = (
            (edges_missing_temporal / total_edges) * 100 if total_edges > 0 else 0
        )
        ...
        if missing_sources_pct > 50:
            self.issues.append(
                QualityIssue(
                    ...
                    evidence={
                        "missing_sources_pct": missing_sources_pct,
                        "missing_count": edges_missing_sources,
                    },
                )
            )
    ...
    def _check_accuracy(self) -> None:
        ...
        try:
            ...
            node_issues = sum(
                ...
                if len(n["name"]) < 2 or n["game"] not in ["MTG", "PKM", "YGO", "DIG", "OP", "RFT"]
            )
            edge_issues = sum(
                1
                for e in sample_edges
                if e["weight"] < 0 or e["weight"] > 100000 or not e["card1"] or not e["card2"]
            )
    ...
    def _calculate_score(self) -> float:
        ...
        unknown_pct = (
            self.statistics.get("unknown_nodes", 0) / max(1, self.statistics.get("total_nodes", 1))
        ) * 100
        if unknown_pct > 10:
            score -= 0.1
        elif unknown_pct > 5:
