    ...
    def _create_agent(self) -> Agent:
        ...
        def check_node_exists(card_name: str) -> dict[str, Any]:
            ...
            try:
                ...
                logger.debug(
                    f"[TOOL] check_node_exists returned: exists={result.get('exists', False)}"
                )
                return result
        ...
        def check_data_integrity() -> dict[str, Any]:
            ...
            try:
                ...
                logger.debug(
                    f"[TOOL] check_data_integrity returned: integrity_score={result.get('integrity_score', 0):.2%}"
                )
                return result
            except Exception as e:
        ...
        def query_edges_by_weight(
            min_weight: float = 0, max_weight: float | None = None, limit: int = 100
        ) -> list[dict[str, Any]]:
...
Be thorough - use multiple tools to build complete picture before concluding."""
        ...
        try:
            ...
            timeout_seconds = float(
                os.getenv("AGENTIC_QA_TIMEOUT_ANALYSIS", "300.0")
            )  # Default 5 minutes
            try:
                result = await asyncio.wait_for(self.agent.run(prompt), timeout=timeout_seconds)
            except TimeoutError:
                timeout_min = timeout_seconds / 60
                logger.error(
                    f"Agent analysis timed out after {timeout_min:.1f} minutes [correlation_id={corr_id}]"
                )
...
- Confidence levels for each finding"""
        ...
        try:
            ...
            timeout_seconds = float(
                os.getenv("AGENTIC_QA_TIMEOUT_COMPREHENSIVE", "900.0")
            )  # Default 15 minutes
            try:
                result = await asyncio.wait_for(self.agent.run(prompt), timeout=timeout_seconds)
            except TimeoutError:
                timeout_min = timeout_seconds / 60
                logger.error(
                    f"Comprehensive analysis timed out after {timeout_min:.1f} minutes [correlation_id={corr_id}]"
                )
                raise TimeoutError(
                    f"Comprehensive analysis timed out after {timeout_min:.1f} minutes"
                )
            ...
            pipeline_summary = self.tools.get_pipeline_summary()
            log_progress(
                logger, "pipeline_validation", progress=2, total=7, stage="pipeline_summary"
            )
            ...
            for i, order in enumerate(sorted(DATA_ORDERS.keys()), start=5):
                try:
                    ...
                    log_progress(
                        ...
                        progress=i,
                        total=7,
                        stage=f"freshness_order_{order}",
                    )
                except Exception as e:
