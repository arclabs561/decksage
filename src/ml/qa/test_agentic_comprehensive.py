...
async def test_comprehensive_agent():
    ...
    try:
        ...
        if isinstance(pipeline, dict):
            ...
            print(
                f"  - Orders with data: {summary.get('orders_with_data', 0)}/{summary.get('total_orders', 7)}"
            )
            ...
            if freshness:
                stale_orders = [
                    order
                    for order, data in freshness.items()
                ...
                if stale_orders:
                    ...
                    for order in stale_orders[:2]:
                        ...
                        if issues:
                            issue = issues[0]
                            print(
                                f"    Order {order}: Stale by {issue.get('stale_by_days', 0):.1f} days"
                            )
            ...
            if agent_analysis:
                ...
                print(
                    f"  - Recommended Fix: {agent_analysis.get('recommended_fix', 'N/A')[:100]}..."
                )
        ...
        print("Issue: Graph has 1000+ orphaned edges")
        print("Analysis:")
        ...
        if pairs_files:
            ...
            if PATHS.incremental_graph_db.exists():
                comparison = agent.tools.compare_file_timestamps(
                    PATHS.incremental_graph_db, pairs_files[0]
                )
                if comparison.get("path1_newer"):
                    print("  - Graph is NEWER than pairs (good)")
                elif comparison.get("path2_newer"):
                    print(
                        f"  - Graph is OLDER than pairs by {comparison.get('age_difference_days', 0):.1f} days (stale!)"
                    )
        print()
