        ...
        if orders_with_issues > 0:
                        ...
                        if order in [2, 3, 4, 5, 6]:  # Downstream orders
                            if any("Order 1" in dep for dep in missing):
                                critical_orders.append(
                                    (order, "Blocked by missing Order 1 (Exported Decks)")
                                )
                            elif any("Order 2" in dep for dep in missing):
                                critical_orders.append(
                                    (order, "Blocked by missing Order 2 (Co-occurrence Pairs)")
                                )
            ...
            if critical_orders:
                ...
                if export_script.exists():
                    ...
                    print(
                        "     → Script location: Check scripts/data_processing/ or src/ml/scripts/"
                    )
...
async def comprehensive_analysis(graph_db: Path | None = None, output: Path | None = None) -> int:
    ...
    try:
        ...
        print("=" * 70)
        print(
            f"Pipeline: {summary.get('orders_with_data', 0)}/{summary.get('total_orders', 7)} orders have data"
        )
