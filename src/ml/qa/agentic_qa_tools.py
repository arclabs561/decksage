...
from ..utils.lineage import DATA_ORDERS, check_dependencies, get_lineage_info
...
class GraphQATools:
    ...
    def check_node_exists(self, card_name: str) -> dict[str, Any]:
        ...
        if similar:
            ...
            result["suggestion"] = (
                f"Node not found. Found {len(similar)} similar nodes (check spelling/game label)"
            )
    ...
    def validate_game_label(
        self, card_name: str, expected_game: str | None = None
    ) -> dict[str, Any]:
        ...
        if detected_game and actual_game:
            ...
            if detected_normalized != actual_game:
                result["valid"] = False
                result["reason"] = (
                    f"Card database suggests {detected_normalized}, graph has {actual_game}"
                )
    ...
    def _find_most_common_missing_card(
        self, orphaned_details: list[dict[str, Any]]
    ) -> dict[str, Any] | None:
    ...
    def check_data_freshness(self, order: int) -> dict[str, Any]:
                ...
                if check_path.exists():
                    if check_path.is_dir():
                        ...
                        if files:
                            newest = max(files, key=lambda p: p.stat().st_mtime)
                            dep_timestamps[
                                f"Order {dep_order} ({dep_info.get('name', 'Unknown')})"
                            ] = newest.stat().st_mtime
                    else:
                        dep_timestamps[f"Order {dep_order} ({dep_info.get('name', 'Unknown')})"] = (
                            check_path.stat().st_mtime
                        )
        ...
        return {
            ...
            "stale_issues": issues,
            "order_timestamps": {
                loc: datetime.fromtimestamp(ts).isoformat() for loc, ts in order_timestamps.items()
            },
            "dependency_timestamps": {
                dep: datetime.fromtimestamp(ts).isoformat() for dep, ts in dep_timestamps.items()
            },
        }
    ...
    def validate_nodes_against_decks(self, game: str | None = None) -> dict[str, Any]:
        ...
        return {
            ...
            "coverage_pct": round(
                (len(graph_nodes & deck_cards) / max(1, len(deck_cards))) * 100, 1
            )
            if deck_cards
            else 0,
        }
