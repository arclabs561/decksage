...
func NewBucket(
	...
	// Create a noop logger if nil is passed (for tests)
	if log == nil {
		log = logger.NewLogger(ctx)
		// Set to panic level to suppress all output in tests
		log.SetLevel("panic")
	}
...
type OptBucketCache struct {
	Dir string
}

func (o *OptBucketCache) bucketOption() {}
...
func (b *Bucket) Close(ctx context.Context) {
	if b.cache != nil {
...
func (b *Bucket) Migrate(ctx context.Context) error {
	it := b.bucket.List(&blob.ListOptions{})
	var found, affected, errs atomic.Int64
	display := func() {
		b.log.Field("found", fmt.Sprintf("%d", found.Load())).
			Field("affected", fmt.Sprintf("%d", affected.Load())).
			Field("errors", fmt.Sprintf("%d", errs.Load())).
			Infof(ctx, "migrate progress")
	}
	wg := new(sync.WaitGroup)
	sem := make(chan struct{}, 48)
	for {
		if n := found.Load(); n > 0 && n%10 == 0 {
			display()
		}
		o, err := it.Next(ctx)
		if err == io.EOF {
		...
		if filepath.Ext(o.Key) == ".json" {
			...
			go func() {
				...
				if err != nil {
					b.log.Field("error", fmt.Sprintf("%v", err)).Errorf(ctx, "failed to read key: %s", o.Key)
					errs.Add(1)
					return
				}
				if err := b.Write(ctx, o.Key, data); err != nil {
					b.log.Field("error", fmt.Sprintf("%v", err)).Errorf(ctx, "failed to write key: %s", o.Key)
					errs.Add(1)
					return
				}
				if err := b.bucket.Delete(ctx, o.Key); err != nil {
					b.log.Field("error", fmt.Sprintf("%v", err)).Errorf(ctx, "faile to delete key: %s", o.Key)
					errs.Add(1)
					return
				}
				affected.Add(1)
