...
def analyze_image_coverage(
    ...
    for card_name, card_dict in card_data.items():
        image_url = embedder._get_image_url(
            card_dict if isinstance(card_dict, dict) else {"name": card_name}
        )
        if image_url:
            cards_with_images += 1
        else:
...
def analyze_by_game() -> dict[str, Any]:
    ...
    for game in games:
        ...
        try:
            ...
            possible_paths = [
                PATHS.data / f"{game}_cards.json",
                PATHS.data / "processed" / f"{game}_cards.json",
                PATHS.data / "raw" / f"{game}_cards.json",
            ]
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Analyze visual embedding coverage across games")
    parser.add_argument(
        "--card-data",
        type=Path,
    ...
    if args.all_games or not args.game:
        ...
        if args.card_data:
            results = {
                args.game: analyze_image_coverage(card_data_path=args.card_data, game=args.game)
            }
        else:
    ...
    # Save results
    if args.output:
        # Order 5: analysis results (derived from embeddings and data)
