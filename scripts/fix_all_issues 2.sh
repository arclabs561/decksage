...
# Step 4: Agentic pipeline validation
echo "Step 4: Running agentic pipeline validation..."
echo "─────────────────────────────────────────────────────────────────────"
if command -v uv >/dev/null 2>&1; then
    if uv run python -m src.ml.qa.validate_agentic --pipeline 2>&1 | tail -20; then
        echo "✓ Agentic validation complete"
    else
        echo "⚠ Agentic validation found issues (see recommendations above)"
    fi
else
    echo "⚠ uv not found, skipping agentic validation"
fi
echo ""
...
echo ""
echo "3. Run agentic comprehensive analysis for detailed recommendations:"
echo "   uv run python -m src.ml.qa.validate_agentic --comprehensive"
echo ""
echo "4. Run reliable evaluation:"
echo "   uv run scripts/evaluation/ensure_full_evaluation.py"
echo ""
echo "5. View comprehensive report:"
