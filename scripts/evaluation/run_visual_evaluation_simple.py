    ...
    try:
        ...
        try:
            ...
            if image_urls_path.exists():
                ...
                logger.info(
                    f"  Loaded {len(card_data)} cards with {sum(1 for c in card_data.values() if c.get('image_url'))} image URLs"
                )
            else:
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Simple visual embeddings evaluation")
    parser.add_argument(
        "--test-set",
        type=Path,
