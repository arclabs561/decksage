...
def filter_test_set_with_images(
    ...
    logger.info(
        f"  Filtered queries: {len(filtered_queries)} ({len(filtered_queries) / len(queries) * 100:.1f}% coverage)"
    )
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Filtered visual embeddings evaluation")
    parser.add_argument(
        "--test-set",
        type=Path,
