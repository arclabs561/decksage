...
def validate_data_files(result: ValidationResult) -> None:
    ...
    for name, path in critical_files.items():
        if path.exists():
            ...
            result.add_fail(f"Data file: {name}", "File not found", f"Check if file exists at {path}")
    ...
    for name, path in test_sets.items():
        ...
        try:
            ...
            if not isinstance(sample_labels, dict):
                result.add_fail(f"Test set: {name}", f"Invalid label format for query '{sample_query}'")
                continue
            ...
            if not has_relevant:
                result.add_warning(f"Test set: {name}", f"Query '{sample_query}' has no relevant labels")
            
            # Check minimum queries
            if num_queries < 10:
                result.add_warning(f"Test set: {name}", f"Only {num_queries} queries (recommend ≥10)")
...
def validate_embeddings(result: ValidationResult) -> None:
    ...
    try:
        ...
        result.add_pass("Embedding format", f"Sample '{sample.name}' loads correctly ({len(embedding)} vectors)")
    except ImportError:
