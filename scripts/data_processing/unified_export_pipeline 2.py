...
def enhance_decks(input_file: Path, output_file: Path) -> dict[str, Any]:
    ...
    # Try to import enhance_exported_decks
    enhance_script = script_dir / "enhance_exported_decks.py"
    if not enhance_script.exists():
        # Try parent directory
        enhance_script = script_dir.parent / "enhance_exported_decks.py"

    if enhance_script.exists():
        import importlib.util

        spec = importlib.util.spec_from_file_location("enhance_exported_decks", enhance_script)
        enhance_module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(enhance_module)
        enhance_func = enhance_module.enhance_decks
        ...
        # Fallback: simple copy with basic stats
        print("Warning: enhance_exported_decks.py not found, using simple copy")
        import shutil
...
def main() -> int:
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Unified export and enhancement pipeline")
    parser.add_argument(
        "--skip-export",
        action="store_true",
