...
def improve_test_set(
    ...
    for query, labels in canonical_queries.items():
        if isinstance(labels, dict):
            total_labels = sum(len(labels.get(level, [])) for level in 
                             ["highly_relevant", "relevant", "somewhat_relevant", "marginally_relevant"])
            if total_labels < 3:
    ...
    for query, canonical_labels in canonical_queries.items():
        ...
        # Count labels before
        total_before = sum(len(canonical_labels.get(level, [])) for level in 
                          ["highly_relevant", "relevant", "somewhat_relevant", "marginally_relevant"])
        ...
        # Count labels after
        total_after = sum(len(improved_labels.get(level, [])) for level in 
                         ["highly_relevant", "relevant", "somewhat_relevant", "marginally_relevant"])
...
def main() -> int:
    ...
    if result["improvements"]:
        ...
        for imp in sorted(result["improvements"], key=lambda x: x["labels_added"], reverse=True)[:10]:
            print(f"  {imp['query']}: {imp['labels_before']} → {imp['labels_after']} labels (+{imp['labels_added']})")
