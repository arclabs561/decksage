...
def update_card_data_with_images(
    ...
    # Filter to test set cards if requested
    if test_cards:
        # Find name column
        name_col = None
        ...
        if name_col:
            # Filter to only test set cards
            df = df[df[name_col].isin(test_cards)]
            logger.info(f"  Filtered to {len(df)} test set cards")
        else:
            logger.warning("Could not find name column for test set filtering")
    ...
    for idx, row in df.iterrows():
        ...
        if (fetched + failed) % 50 == 0:
            logger.info(
                f"  Progress: {fetched} fetched, {failed} failed, {already_had} already had URLs"
            )
...
def main() -> int:
    """CLI entry point."""
    parser = argparse.ArgumentParser(description="Update card data with image URLs from Scryfall")
    parser.add_argument(
        "--card-attrs",
        type=Path,
    ...
    if args.test_set_only:
        try:
            from ml.utils.annotation_utils import load_test_set_cards
            from ml.utils.paths import PATHS

            # Load all test set cards (queries + all relevant cards)
            test_cards = load_test_set_cards(test_set_path=PATHS.test_magic, game="magic")
            logger.info(f"Test set has {len(test_cards)} unique cards (queries + relevant)")
        except ImportError:
            ...
            if test_set_path.exists():
                ...
                logger.info(f"Test set has {len(test_cards)} unique cards (fallback method)")
            else:
