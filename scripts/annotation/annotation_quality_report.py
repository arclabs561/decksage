...
def analyze_annotation_quality(annotations_dir: Path) -> dict:
    ...
    annotation_files = [
        f for f in annotation_files if "enriched" not in str(f) and ".bak" not in str(f)
    ]
...
def print_report(report: dict) -> None:
    ...
    print(
        f"  Valid: {summary['valid_annotations']} ({summary['valid_annotations'] / summary['total_annotations'] * 100:.1f}%)"
        if summary["total_annotations"] > 0
        else "  Valid: 0"
    )
    print(
        f"  Invalid: {summary['invalid_annotations']} ({summary['invalid_annotations'] / summary['total_annotations'] * 100:.1f}%)"
        if summary["total_annotations"] > 0
        else "  Invalid: 0"
    )
    print()
    ...
    if summary["issues"]:
        ...
        for issue, count in sorted(summary["issues"].items(), key=lambda x: x[1], reverse=True):
            pct = (
                (count / summary["total_annotations"] * 100)
                if summary["total_annotations"] > 0
                else 0
            )
    ...
    for filename, file_report in sorted(report["files"].items()):
        ...
        print(
            f"    Total: {file_report['total']}, Valid: {file_report['valid']}, Invalid: {file_report['invalid']}"
        )
    ...
    if summary["issues"]:
        ...
        if summary["issues"].get("missing_source"):
            print(
                "  1. Run: uv run python3 scripts/annotation/fix_missing_fields.py --input-dir annotations/"
            )
        if summary["issues"].get("missing_similarity_score"):
            print(
                "  2. Check Yu-Gi-Oh annotations - they may use 'confidence' instead of 'similarity_score'"
            )
        print("  3. Re-generate annotations with updated llm_annotator.py (includes source field)")
        print(
            "  4. Re-enrich annotations after fixing: uv run python3 scripts/annotation/enrich_existing_annotations.py"
        )
