...
def main() -> int:
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Analyze quality of annotation enrichment")
    parser.add_argument(
        "--input",
        type=Path,
        required=True,
    ...
    print(
        f"  Enriched: {analysis['enrichment_coverage']['total_enriched']} ({analysis['enrichment_coverage']['enrichment_rate']:.1%})"
    )
    print()
