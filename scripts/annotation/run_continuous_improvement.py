...
from src.ml.annotation.meta_judge import inject_context_into_annotator, meta_judge_annotations
...
async def generate_and_improve(
    ...
    for iteration in range(iterations):
            ...
            try:
                ...
                if feedback and feedback.get("issues"):
                    ...
                    for issue in feedback["issues"][:3]:  # Show first 3
                        print(
                            f"    - {issue.get('type', 'unknown')}: {issue.get('description', '')[:60]}"
                        )

                # Inject context for next iteration
                if feedback:
                    inject_context_into_annotator(annotator, feedback)
                    print("  ✅ Context injected for next iteration")
        ...
        # Save annotations
        output_file = (
            project_root / "annotations" / f"{game}_improvement_iter_{iteration + 1}.jsonl"
        )
