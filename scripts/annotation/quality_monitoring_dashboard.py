    ...
    if hand_stats:
        ...
        if overall_completion == 0.0:
            ...
            report["recommendations"].append(
                "Complete at least one batch using browser annotation tool"
            )
    ...
    if llm_stats:
        ...
        if uniform_score_issues > 0:
            report["issues"].append(
                f"{uniform_score_issues} LLM annotation files have uniform scores (suspicious)"
            )
            report["recommendations"].append(
                "Regenerate LLM annotations using generate_llm_annotations.py"
            )
...
def main() -> int:
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Annotation quality monitoring dashboard")
    parser.add_argument(
        "--annotations-dir",
        type=Path,
