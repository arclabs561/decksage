...
def benchmark_full_loading(num_queries: int = 10) -> dict:
    ...
    for card1, card2 in test_pairs[:num_queries]:
        ...
        if features:
            print(
                f"  {card1} ↔ {card2}: {query_time * 1000:.1f}ms (Jaccard: {features.jaccard_similarity:.3f})"
            )
...
def main() -> int:
    ...
    if "error" not in full_results and "error" not in lazy_results:
        ...
        print("  Memory: ~2.1GB (all data in memory)")
        ...
        print("  Memory: <100MB (queries only)")

        speedup = (
            full_results["total_time"] / lazy_results["total_time"]
            if lazy_results["total_time"] > 0
            else 0
        )
        print(f"\n  Speedup: {speedup:.2f}x faster with lazy loading")
        print("  Memory savings: ~95% reduction")
