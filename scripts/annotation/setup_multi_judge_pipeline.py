...
def setup_pipeline_for_queries(
    ...
    # Save summary
    summary_file = (
        output_dir / f"multi_judge_pipeline_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
...
def main() -> int:
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Set up multi-judge annotation pipeline")
    parser.add_argument(
        "--query",
        type=str,
