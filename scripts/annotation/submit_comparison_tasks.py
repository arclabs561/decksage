        ...
        try:
            ...
            try:
                ...
                if balance["available"] < 0.12 * num_tasks:
                    ...
                    print("  Add balance: https://requester.mturk.com/account")
            ...
            for card1, card2, game, reason in test_pairs:
                try:
                    task_id = (
                        f"compare_mturk_{card1}_{card2}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
                    )
    ...
    if submit_scale:
        ...
        try:
            ...
            for card1, card2, game, reason in test_pairs:
                try:
                    task_id = (
                        f"compare_scale_{card1}_{card2}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
                    )
