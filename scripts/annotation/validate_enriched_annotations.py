...
def main() -> int:
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Validate enriched annotations against graph data")
    parser.add_argument(
        "--input",
        type=Path,
        required=True,
    ...
    print(
        f"  With graph features: {summary['with_graph_features']} ({summary['enrichment_rate']['graph']:.1%})"
    )
    print(
        f"  With card attributes: {summary['with_card_attributes']} ({summary['enrichment_rate']['attributes']:.1%})"
    )
    print(
        f"  With contextual analysis: {summary['with_contextual_analysis']} ({summary['enrichment_rate']['context']:.1%})"
    )
    ...
    if "jaccard_stats" in summary:
        ...
        print(
            f"  Range: {summary['jaccard_stats']['min']:.3f} - {summary['jaccard_stats']['max']:.3f}"
        )
    ...
    if "cooccurrence_stats" in summary:
        ...
        print(
            f"  Range: {summary['cooccurrence_stats']['min']} - {summary['cooccurrence_stats']['max']}"
        )
