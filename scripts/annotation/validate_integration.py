...
try:
    ...
    def extract_substitution_pairs_from_annotations(annotations, **kwargs):
        ...
        for ann in annotations:
            if ann.get("is_substitute") and ann.get("similarity_score", 0) >= kwargs.get(
                "min_similarity", 0.8
            ):
...
def validate_test_set(test_set_file: Path) -> dict:
    ...
    total_relevant = sum(
        len(v.get("highly_relevant", [])) + len(v.get("relevant", [])) for v in queries.values()
    )
...
def main() -> int:
    ...
    all_valid = all(r.get("valid", False) for r in results.values() if isinstance(r, dict))
