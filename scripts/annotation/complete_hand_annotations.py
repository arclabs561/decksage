...
def create_browser_interfaces_for_batch(
    ...
    for task in tasks:
        ...
        html_file = (
            output_dir / f"browser_annotation_{query.replace(' ', '_').replace('/', '_')}.html"
        )
        ...
        try:
            ...
            query_count += 1
            print(
                f"  Created interface for query {query_count}: {query} ({len(candidate_names)} candidates)"
            )
...
def main() -> int:
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Complete hand annotations using browser tool")
    parser.add_argument(
        "--batch",
        type=Path,
