...
async def test_multi_annotator_iaa(game: str = "magic", num_pairs: int = 2):
    ...
    try:
        ...
        print(
            f"  ✅ Multi-annotator initialized with {len(annotator.multi_annotator.annotator_configs)} annotators"
        )
...
async def test_uncertainty_selection(game: str = "magic", num_pairs: int = 2):
    ...
    try:
        ...
        if scores:
            ...
            print(
                f"    Score std: {(sum((s - sum(scores) / len(scores)) ** 2 for s in scores) / len(scores)) ** 0.5:.3f}"
            )
