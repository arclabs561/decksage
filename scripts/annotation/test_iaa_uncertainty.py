...
async def test_multi_annotator_iaa():
    ...
    try:
        ...
        print("✓ Multi-annotator annotation completed")
...
async def test_uncertainty_selection():
    ...
    try:
        ...
        for card1, card2 in candidate_pairs:
            ...
            print(
                f"    {card1} vs {card2}: uncertainty={uncertainty.uncertainty_score:.3f} ({uncertainty.uncertainty_type})"
            )
    ...
    try:
        ...
        for ann in annotations_uncertainty:
            if isinstance(ann, dict):
                print(
                    f"  {ann.get('card1')} vs {ann.get('card2')}: score={ann.get('similarity_score', 0.0):.3f}"
                )
            else:
        ...
        for ann in annotations_iaa:
            if isinstance(ann, dict):
                print(
                    f"  {ann.get('card1')} vs {ann.get('card2')}: score={ann.get('similarity_score', 0.0):.3f}, source={ann.get('source', 'unknown')}"
                )
            else:
                print(
                    f"  {ann.card1} vs {ann.card2}: score={ann.similarity_score:.3f}, source={ann.source}"
                )
