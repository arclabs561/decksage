...
def load_card_attributes() -> dict[str, dict] | None:
    ...
    attrs_path = (
        Path(__file__).parent.parent.parent / "data" / "processed" / "card_attributes_enriched.csv"
    )
...
def profile_enrichment():
    ...
    try:
        with LazyGraphEnricher(graph_path, game="MTG") as enricher:
            ...
            print(
                f"  get_neighbors: {neighbor_time * 1000:.1f}ms (card1: {len(neighbors1)}, card2: {len(neighbors2)})"
            )
