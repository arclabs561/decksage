...
def analyze_all_files(annotations_dir: Path) -> dict:
    ...
    similarity_scores = [
        a.get("similarity_score") for a in all_annotations if a.get("similarity_score") is not None
    ]

    # Graph feature coverage
    has_cooccurrence = sum(
        1 for a in all_annotations if a.get("graph_features", {}).get("cooccurrence_count", 0) > 0
    )
    has_common_neighbors = sum(
        1 for a in all_annotations if a.get("graph_features", {}).get("common_neighbors", 0) > 0
    )
...
def print_analysis(analysis: dict) -> None:
    ...
    if sim["min"] is not None:
        ...
        if sim["unique_values"] == 1:
            print("  ⚠️  WARNING: All annotations have same similarity score!")
    print()
