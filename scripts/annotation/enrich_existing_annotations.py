...
def enrich_annotations_file(
    ...
    print(
        f"  Using {'lazy graph loading' if lazy_graph_path else 'full graph' if graph else 'no graph'}"
    )
    if lazy_graph_path:
    ...
    if lazy_graph_path and lazy_graph_path.exists():
        try:
            ...
            print("  ✓ Reusing graph connection for all annotations")
        except Exception as e:
            print(f"  ⚠️  Could not initialize lazy enricher: {e}")
            lazy_enricher = None

    try:
        with open(input_path) as f_in, open(temp_file, "w") as f_out:
            for line_num, line in enumerate(f_in, 1):
                ...
                try:
                    ...
                    should_log = (
                        total_count % 5 == 0 or total_count == 1 or time_since_last_log >= 3.0
                    )

                    if should_log:
                        rate = total_count / elapsed if elapsed > 0.1 else 0
                        pct_enriched = (
                            (enriched_count / total_count * 100) if total_count > 0 else 0
                        )
                        print(
                            f"  [{total_count:4d}] Enriched: {enriched_count:4d} ({pct_enriched:5.1f}%) | "
                            f"Time: {elapsed:6.1f}s | Rate: {rate:5.2f} ann/s",
                            flush=True,
                        )
                    ...
                    if lazy_enricher:
                        try:
                            ...
                            if annotation.get("card1") and annotation.get("card2"):
                                ...
                                enriched = enrich_annotation_with_graph(
                                    annotation,
                                    None,
                                    card_attributes,  # Pass None for graph
                                )
                                ...
                                if graph_features:
                                    ...
                                    if (
                                        total_count % 25 == 0
                                        and graph_features.cooccurrence_count > 0
                                    ):
                            ...
                            print(
                                f"  [{total_count}] ⚠️  Lazy enrichment failed for {card1}↔{card2}: {e}",
                                flush=True,
                            )
                        ...
                        enriched = enrich_annotation_with_graph(annotation, graph, card_attributes)

                    # Check if enrichment added data
                    has_enrichment = bool(
                        enriched.get("graph_features")
                        or enriched.get("card_comparison")
                        or enriched.get("contextual_analysis")
                    )
                    if has_enrichment:
                        enriched_count += 1
    ...
    print(
        f"Enriched: {enriched_count} ({enriched_count / total_count * 100:.1f}%)"
        if total_count > 0
        else "No annotations processed"
    )
