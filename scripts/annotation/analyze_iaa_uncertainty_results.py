...
def analyze_results(results_file: Path):
    ...
    if "uncertainty" in results and "error" not in results["uncertainty"]:
        ...
        if "single" in results and "error" not in results["single"]:
            ...
            print("\n  vs Single Annotator:")
            print(
                f"    Mean difference: {mean_diff:+.3f} ({mean_diff / single['score_mean'] * 100:+.1f}%)"
            )
            print(f"    Std difference: {std_diff:+.3f}")

            # Score diversity improvement
            single_low = (
                single["score_distribution"].get("very_low (0.0-0.2)", 0)
                / single["num_annotations"]
            )
            uncertainty_low = (
                uncertainty["score_distribution"].get("very_low (0.0-0.2)", 0)
                / uncertainty["num_annotations"]
            )
            diversity_improvement = single_low - uncertainty_low
            print(
                f"    Low-score reduction: {diversity_improvement * 100:.1f}% (fewer very low scores)"
            )
        print()
