...
def test_similarity_methods(
    base_url: str, query: str = "Lightning Bolt", top_k: int = 5
) -> dict[str, Any]:
...
def test_performance_consistency(
    base_url: str, query: str = "Lightning Bolt", num_runs: int = 5
) -> dict[str, Any]:
...
def main():
    ...
    if args.verbose and diag_results.get("status") == "pass":
        ...
        if weights:
            print(
                f"    Fusion weights: {', '.join(f'{k}={v:.3f}' for k, v in weights.items() if v > 0)}"
            )
    ...
    for method, result in method_results.items():
        ...
        print(
            f"  {status} {method}: {result.get('status', 'unknown')} ({result.get('num_results', 0)} results)"
        )
    ...
    for agg, result in agg_results.items():
        status = "✓" if result.get("status") == "pass" else "✗"
        print(
            f"  {status} {agg}: {result.get('status', 'unknown')} ({result.get('num_results', 0)} results)"
        )
