...
def validate_substitution_pairs(
    ...
    warnings = []
    if not is_valid:
        warnings.append(f"CRITICAL: {len(leaked_pairs)} substitution pairs contain test set cards.")
...
def main() -> int:
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Validate training data for data leakage")
    parser.add_argument(
        "--annotations",
        type=Path,
    ...
    if args.output:
        ...
        # Write with lineage validation (Order 6: Annotations/Validation results)
        from ml.utils.lineage import safe_write

        with safe_write(args.output, order=6, strict=False) as validated_path:
