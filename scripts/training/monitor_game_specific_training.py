...
try:
    from ml.utils.log_monitor import LocalLogMonitor, format_status  # noqa: F401
...
def check_training_status(game, use_structured=True):
    ...
    if use_structured and HAS_STRUCTURED and log.exists():
        try:
            ...
            if structured_status.stage:
                ...
                status["progress"] = (
                    f"Metrics: {', '.join(f'{k}={v}' for k, v in list(structured_status.last_metric.items())[:2])}"
                )
    ...
    for line in reversed(recent_lines):
        if "[PROGRESS]" in line or "EPOCH" in line:
            status["progress"] = line.strip()[:80]
            break
