...
def main() -> int:
    ...
    if not isinstance(test_set, dict):
        print(f"Error: Test set format invalid")
        return 1
    ...
    for emb_path in sorted(embeddings_to_test):
        ...
        try:
            ...
            print(f"✓ P@10={result['p@10']:.4f}, {result.get('num_evaluated', 0)}/{len(test_set)} queries")
    ...
    with open(args.output, "w") as f:
        json.dump({
            ...
            "summary": {
                ...
                "best_p@10": max([r.get('p@10', 0) for r in results.values()]) if results else 0.0,
            },
            "results": results,
        }, f, indent=2)
