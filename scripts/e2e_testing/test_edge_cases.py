    ...
    def test_empty_query(self):
        ...
        try:
            ...
            error_msg = self.page.locator(
                "[class*='error'], [class*='status'], :has-text('required')"
            )
    ...
    def test_rapid_typing(self):
        ...
        try:
            ...
            if dropdown.count() > 0 and dropdown.first.is_visible():
                ...
                logger.info(
                    "  ℹ️  Rapid typing handled (autocomplete may not appear if API has no data)"
                )
    ...
    def test_network_error(self):
        ...
        try:
            ...
            if error_msg.count() > 0 and error_msg.is_visible():
                ...
                empty_state = self.page.locator(
                    ".empty-state, [class*='empty'], :has-text('No results'), :has-text('not found')"
                )
        ...
        try:
            ...
            if empty_state.count() > 0:
                ...
                if results.count() == 0:
                    ...
                    logger.info(
                        "  ℹ️  Invalid card name returned results (may be valid or API has data)"
                    )
                    return True  # Don't fail
        except Exception as e:
    ...
    def test_tab_switching(self):
        ...
        try:
            ...
            if visible_count == 1:  # Exactly one panel should be visible
                ...
                logger.warning(
                    f"  ⚠️  Tab switching may not work correctly ({visible_count} panels visible)"
                )
                return False
        except Exception as e:
