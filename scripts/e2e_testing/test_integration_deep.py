...
def test_end_to_end_flow():
    ...
    try:
        ...
        found_fields = [f for f in metadata_fields if metadata.get(f)]
        logger.info(f"✅ Metadata: {len(found_fields)}/{len(metadata_fields)} fields")

        # Step 4: Verify graph/archetype data
        has_graph_data = any(
            f in metadata
            for f in ["archetype_staples", "cooccurrence_note", "archetype_cooccurrence"]
        )
        if has_graph_data:
...
def test_fallback_behavior():
    ...
    try:
        ...
        assert resp.status_code == 200, f"Expected 200, got {resp.status_code} for fallback test"
        data = resp.json()
...
def test_metadata_completeness():
    ...
    assert coverage >= 50, f"Expected at least 50% metadata coverage, got {coverage:.1f}%"
    return coverage >= 50
