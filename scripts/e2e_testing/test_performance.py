...
import requests
from test_constants import PERF_PARAMS, RESULT_SIZES, TEST_CARDS, TIMEOUTS
    ...
    for name, (url, max_time) in benchmarks.items():
        ...
        for _ in range(PERF_PARAMS["benchmark_iterations"]):
            ...
            try:
                if "similar" in url:
                    resp = requests.post(
                        url, json={"query": TEST_CARDS["common"], "top_k": 5}, timeout=max_time * 2
                    )
                else:
                    resp = requests.get(url, timeout=max_time * 2)
                elapsed = time.time() - start
                assert resp.status_code == 200, f"Expected 200, got {resp.status_code} for {name}"
...
def test_throughput():
    ...
    passed = successful >= num_requests * 0.9 and avg_time < 0.5 and throughput > 10
...
def test_concurrent_load():
    ...
    if times:
        ...
        passed = successful >= num_concurrent * 0.8 and avg_time < 2.0 and p95_time < 5.0
    else:
