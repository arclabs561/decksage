...
import requests
from test_constants import SQL_INJECTION_PAYLOADS, TEST_CARDS, TIMEOUTS, XSS_PAYLOADS
...
def test_xss_prevention():
    ...
    for payload in XSS_PAYLOADS:
        try:
            ...
            resp = requests.get(
                f"{API_BASE}/v1/cards?prefix={payload}&limit=5", timeout=TIMEOUTS["normal"]
            )
            # Handle 404 or 503 gracefully (service may not be ready)
            if resp.status_code == 404:
                logger.warning("⚠️  XSS test endpoint not found (404) - skipping")
                return False
            elif resp.status_code == 503:
                logger.warning(
                    "⚠️  XSS test service unavailable (503) - embeddings may not be loaded"
                )
                return False
            ...
            assert payload not in resp.text, f"XSS payload found in response: {payload[:30]}..."
            passed += 1
...
def test_sql_injection():
    ...
    logger.info(
        f"Result: {passed}/{len(SQL_INJECTION_PAYLOADS)} SQL injection attempts handled safely"
    )
...
def test_rate_limiting():
    ...
    for i in range(50):  # Send 50 requests rapidly
        try:
            resp = requests.get(
                f"{API_BASE}/v1/cards?prefix=Light&limit=5", timeout=TIMEOUTS["fast"]
            )
            requests_sent += 1
...
def test_input_validation():
    ...
    for invalid_input, description in invalid_inputs:
        try:
            resp = requests.get(
                f"{API_BASE}/v1/cards?prefix={invalid_input}&limit=5", timeout=TIMEOUTS["normal"]
            )
...
def test_json_injection():
    ...
    for payload in malicious_json:
        try:
            resp = requests.post(
                f"{API_BASE}/v1/similar", json=json.loads(payload), timeout=TIMEOUTS["normal"]
            )
            ...
            assert "polluted" not in resp.text.lower(), "Possible prototype pollution detected"
            passed += 1
