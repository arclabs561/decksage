...
# 4. Agentic pipeline validation
echo "4. Running agentic pipeline validation..."
echo "   ──────────────────────────────────────────────────────────────"
if uv run python -m src.ml.qa.validate_agentic --pipeline 2>&1 | tail -15; then
    echo "   ✓ Agentic validation complete"
else
    echo "   ⚠ Agentic validation found issues (see output above)"
fi
echo ""
