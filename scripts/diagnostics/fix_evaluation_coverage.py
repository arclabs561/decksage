...
def main() -> int:
    ...
    for emb_path in sorted(embeddings_to_check):
        ...
        try:
            ...
            if coverage["coverage"] >= 0.8:
                print(f"✓ {coverage['coverage']:.1%} ({coverage['in_vocab']}/{coverage['total_queries']})")
    ...
    working = [
        ...
        if "coverage" in r and r["coverage"] >= 0.8
    ]
    ...
    with open(args.output, "w") as f:
        json.dump({
            ...
            "summary": {
                ...
                "working": len([r for r in results.values() if "coverage" in r and r["coverage"] >= 0.8]),
                "partial": len([r for r in results.values() if "coverage" in r and 0.5 <= r["coverage"] < 0.8]),
                "broken": len([r for r in results.values() if "coverage" in r and r["coverage"] < 0.5]),
            },
            "results": results,
        }, f, indent=2)
