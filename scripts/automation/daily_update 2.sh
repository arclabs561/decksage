...
if [[ "$DRY_RUN" == "true" ]]; then
 ...
 echo "[DRY-RUN] Would run: ml.scripts.update_graph_incremental --graph-path $GRAPH_PATH --decks-path $NEW_DECKS_PATH"
    ...
    --new-decks "$NEW_DECKS_PATH" \
    --post-fix \
    --api-fallback \
    2>&1 | tee -a "$LOG_FILE"
    echo "✓ Graph updated with quality fixes"
else
...
echo "Step 5: Validating pipeline health..."
if [[ "$DRY_RUN" == "true" ]]; then
    echo "[DRY-RUN] Would run agentic pipeline validation"
    echo "[DRY-RUN] Would run: python -m src.ml.qa.validate_agentic --pipeline"
else
    if uv run python -m src.ml.qa.validate_agentic --pipeline 2>&1 | tee -a "$LOG_FILE" | tail -15; then
        echo "✓ Pipeline validation complete"
    else
        echo "⚠️  Pipeline validation found issues (see output above)"
        echo "   Run 'just qa-comprehensive-analysis' for detailed recommendations"
    fi
fi
echo ""
