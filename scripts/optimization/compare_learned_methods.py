...
def main():
    ...
    equal_result = evaluate_weights(
        test_set, embeddings, adj, card_attrs, equal_weights, "rrf", args.top_k
    )
    ...
    if args.grid_search_results and Path(args.grid_search_results).exists():
        ...
        grid_result = evaluate_weights(
            test_set, embeddings, adj, card_attrs, grid_weights, "rrf", args.top_k
        )
    ...
    for learned_file in learned_files:
        ...
        try:
            ...
            result = evaluate_weights(
                test_set, embeddings, adj, card_attrs, weights, "rrf", args.top_k
            )
            comparison[method] = {
                "weights": weights,
                "result": result,
    ...
    for method, data in sorted(
        comparison.items(), key=lambda x: x[1]["result"]["p_at_k"], reverse=True
    ):
        result = data["result"]
