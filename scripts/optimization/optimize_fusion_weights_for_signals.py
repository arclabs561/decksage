...
def grid_search_available_signals(
    ...
    if len(active_weights) > 0:
        ...
        weights_dict = dict.fromkeys(["embed", "jaccard", "functional", "text_embed", "gnn"], 0.0)
        ...
        if evaluated == 0:
            ...
            if len(results) == 0:  # Only print for first result
                print(
                    f"  Evaluated {evaluated} queries, skipped {skipped}, mean P@{top_k}: {p_at_10:.4f}"
                )
        results.append(
            {
    ...
    return {
        "best_weights": {w: getattr(best_weights, w) for w in active_weights}
        if best_weights
        else {},
...
def main():
    ...
    output_path = (
        Path(args.output)
        if args.output
        else PATHS.experiments / "optimized_fusion_weights_latest.json"
    )
    output_data = {
        "signals": signals,
