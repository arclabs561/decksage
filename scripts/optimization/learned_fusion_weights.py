...
def main():
    parser = argparse.ArgumentParser(
        description="Learn optimal fusion weights using gradient boosting"
    )
    ...
    parser.add_argument(
        ...
        help="Aggregator for evaluation",
    )
    ...
    print(
        f"  Equal weights P@{args.top_k}: {baseline_result['p_at_k']:.4f} ± {baseline_result['std']:.4f}"
    )

    improvement = eval_result["p_at_k"] - baseline_result["p_at_k"]
    improvement_pct = (
        (improvement / baseline_result["p_at_k"] * 100) if baseline_result["p_at_k"] > 0 else 0.0
    )
